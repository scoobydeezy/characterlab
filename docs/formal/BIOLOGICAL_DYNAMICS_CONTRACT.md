...
## Two-lane integration clarification before freeze

The integrated host invokes elapsed then consequence phases, both within one atomic
instant. Elapsed advances the clock/phase and passive kinetics, admits no external
impulse and performs no adaptation mutation. Consequence holds clock/phase fixed,
applies action/external pulses without repeating decay, and stages adaptation once.
A separate consequenceState retains pre-adaptation target/tolerance/sensitization
with post-effect physical levels for consequence120 sensing; updated adaptation is
committed at140 and can first affect the next instant. Sleep action reduces debt by
sleepRecovery after current-lane accumulation; no future sleep choice is anticipated.
The default whole-step API is a world-only challenge, not a substitute for this ordering.
