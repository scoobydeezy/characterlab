# relationship-public/0.2-candidate
...
through437 into two407 nuclei using the inherited die bands and activation threshold0,
with standing and situational modifier unit1/4 and cap3 (the AFFECT/WORKSPACE precedent). No raw trust score
...
The0.1 cohort remains frozen as a failed behavioral control: its inherited task dice
disabled both modifiers, so changed history altered appraisal but not response.
The0.2 successor changes only the declared version and modifier calibration, retaining
allocation schema, source inputs, ownership, transition laws and ordering. Both models
remain preparable and replayable. No new architecture edge or psychological law is
selected; calibration remains a bounded candidate. See RELATIONSHIP_IMPLEMENTATION_FINDINGS.md.
