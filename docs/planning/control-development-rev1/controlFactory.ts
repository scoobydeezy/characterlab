...
import freeze from '../../docs/planning/campaign3-control-model-rev1/FREEZE.json';
...
async function constructRun(handle:ControlModel,input:{initialState:Uint8Array;orderedInputs:Uint8Array;runSeed:Uint8Array}){
 ...
 const run=Object.freeze({observerView(i=0){if(i!==0)throw Error('CONTROL_OBSERVER_HANDLE');return enc(list(runtime.snapshot().outputs.filter(v=>typeof v!=='boolean'&&v.kind==='record'&&[826n,827n,828n,829n,830n,831n,832n,836n,837n,838n,1015n,1016n].includes(v.schema.typeId))));},settleNextInstant:async()=>(await runtime.settle())!==undefined,snapshot(){const s=runtime.snapshot();return {clock:s.clock,status:s.status,state:enc(s.state.canonicalValue()),outputs:enc(list(s.outputs)),trace:enc(list(s.trace))};},save:()=>runtime.save(),runIdentity:()=>original.runIdentity.canonicalBytes.slice(),diagnostic(){const d=runtime.diagnostic();return d?enc(failureDiagnosticValue(original.runIdentity.value,d)):undefined;}});
 return {run,runtime};
}
