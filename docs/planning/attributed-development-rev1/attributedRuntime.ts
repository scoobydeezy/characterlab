...
export function createAttributedRuntime(model:AttributedCompiled,input:Awaited<ReturnType<typeof compileAttributedInputs>>){
 ...
 async function handler({event,state,allocateRuntimeId}:EventHandlerContext<AuthoritativeState>){
  ...
  if(name==='source'){
   ...
   const pair=items(event.payload,'list'),intent=intentOutput(occurrence(1136),pair[0]),context=rec(pair[1],1089n),purpose=key(f(chosenData(pair[0]),1n))===key(OPTIONS[0])?1:2,assertion=purpose===1?claim(f(context,2n)):list([]),out=r(1138,[occurrence(),intent,context,u(purpose),assertion]);outputs.push(intent,out);emit('expression',out);
  }else if(name==='expression'){
   const expression=expressionOutput(occurrence(1137),f(rec(event.payload,1090n),2n)),out=r(1139,[occurrence(),event.payload,expression]);outputs.push(expression,out);emit('plan',out);
  }else if(name==='plan'){
   ...
   const attempt=rec(event.payload,1092n),expression=rec(f(attempt,3n),1091n),intent=rec(f(expression,2n),1090n),data=chosenData(f(rec(f(intent,2n),425n),2n)),action=key(f(data,1n))===key(OPTIONS[0])?1:2,value=f(intent,5n),delivered=items(value,'list').length>0&&f(world.get(event.dueAt)!,9n)===true,out=r(1141,[occurrence(),attempt,u(action),delivered,delivered?value:list([])]);outputs.push(out);emit('receive-target',out);
