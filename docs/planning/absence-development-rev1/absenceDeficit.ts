...
export function absenceResponse(law:AbsenceLaw,displacement:bigint,baseline:Q,support:Q){
 if(!LAWS.includes(law)||displacement<0n||displacement>10n||baseline.compare(Q.of(70n))<0||baseline.compare(Q.of(90n))>0||support.compare(Q.of(0n))<0||support.compare(Q.of(10n))>0)throw Error('absence challenge domain');
 const reference=Q.of(80n+(law==='FixedReference'?0n:displacement)),level=baseline.add(support),raw=reference.subtract(level),gap=raw.compare(Q.of(0n))>0?raw:Q.of(0n);
