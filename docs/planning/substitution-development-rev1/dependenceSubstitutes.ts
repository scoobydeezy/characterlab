...
export type Row={at:number;mode:number;demand:number;cue:boolean;available:boolean[];beliefBefore:(string|null)[];habitBefore:string[];admitted:boolean[];values:string[];context:string;raw:string;reasons:string;resolution:string;expression:string;intent:number;attempt:string;receipt:boolean|null};
export function createSubstitutionRun(law:Law,inputs:readonly Frame[],seed=7){
 ...
 let at=0,journal:Practice[]=[],belief:Sample[][]=[[],[]],rows:Row[]=[],physical=[0,0],unserved=0,world:{at:number;execution:string;physical:number[];unserved:number;privateBit:boolean}[]=[],busy=false;
 ...
 return Object.freeze({snapshot,save,observerView:()=>clone({journal,belief,rows}),
  async step(fault?:'after-decision'|'before-commit'){
   ...
   try{
    ...
    const chosen=OPTIONS.findIndex(option=>key(option)===key(f(chosenData(resolution),1n)));if(chosen<0)throw Error('SUBSTITUTION_CHOICE');
    const intent=intentOutput(occ(1136),resolution),expression=expressionOutput(occ(1137),intent),attempt=attemptOutput(occ(1140),planOutput(occ(1139),intent,r(391,[u(1)])));
    if(fault==='after-decision')throw Error('SUBSTITUTION_INJECTED');
    const execution=executionOutput(occ(1141),attempt,chosen<2&&(chosen===0?o.actualA:o.actualB)),success=uint(f(rec(execution,433n),3n))===1n,nextPhysical=[...physical];if(success)nextPhysical[chosen]++;
    const nextUnserved=unserved+(mode===3&&demand===1&&!success?1:0),receipt=chosen<2&&o.receipt?(o.report===0?success:o.report===1):null,nextBelief=clone(belief),nextJournal=clone(journal);
    if(law!=='NoLearning'){
    ...
    const row:Row={at:next,mode,demand,cue,available,beliefBefore:estimates.map(v=>v?qs(v):null),habitBefore:hs.map(qs),admitted,values:values.map(qs),context:key(context),raw:key(raw),reasons:key(reasons),resolution:key(resolution),expression:key(expression),intent:chosen,attempt:key(attempt),receipt};
    if(fault==='before-commit')throw Error('SUBSTITUTION_INJECTED');random.prepareCommit();random.commit();at=next;journal=nextJournal;belief=nextBelief;physical=nextPhysical;unserved=nextUnserved;rows=[...rows,row];world=[...world,{at:next,execution:key(execution),physical:[...physical],unserved,privateBit:o.privateBit}];return true;
