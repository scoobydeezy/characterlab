...
it('learns a mixed cooperative impression from actual conduct while misreading current intent',async()=>{
 const s=await run(),a=apps(s).at(-1)!,b=apps(s,2).at(-1)!;expect(f(a,5n)).toEqual(list([q(2,3)]));expect(f(a,6n)).toEqual(list([q(0,1)]));expect(f(b,5n)).toEqual(f(a,5n));expect(f(b,6n)).toEqual(list([q(1,1)]));expect(f(outputs(s,1155n).at(-1)!,5n)).toEqual(list([true]));expect(f(a,9n)).toEqual(list([q(1,1),q(1,1)]));expect(f(b,9n)).toEqual(list([q(0,1),q(0,1)]));
 expect(items(f(knowledge(s,1),1n),'list')).toHaveLength(3);
});
it('same history with changed current cue changes appraisal without rewriting disposition or old appraisal',async()=>{
 const base=await run(),changed=await run('cueChanged');expect(items(f(knowledge(base,1),1n),'list')).toEqual(items(f(knowledge(changed,1),1n),'list'));expect(f(apps(base).at(-1)!,5n)).toEqual(f(apps(changed).at(-1)!,5n));expect(f(apps(changed).at(-1)!,9n)).toEqual(list([q(0,1),q(0,1)]));expect(apps(base).slice(0,3)).toEqual(apps(changed).slice(0,3));expect(outputs(base,425n)).toEqual(outputs(changed,425n));
});
it('goal-only change preserves estimates and evidence but changes adverse appraisal',async()=>{
 const s=await run(),opposite=await run('main',1,1,2);expect(knowledge(s,1)).toEqual(knowledge(opposite,1));for(const k of [5n,6n])expect(f(apps(s).at(-1)!,k)).toEqual(f(apps(opposite).at(-1)!,k));expect(f(apps(opposite).at(-1)!,9n)).toEqual(list([q(0,1),q(0,1)]));expect(outputs(s,425n)).toEqual(outputs(opposite,425n));
});
it('pooled estimate and single-channel controls retain distinct predictions',async()=>{
 const pooled=await run('main',2),history=await run('main',3),cue=await run('main',4);expect(f(apps(pooled).at(-1)!,5n)).toEqual(list([q(1,2)]));expect(f(apps(pooled).at(-1)!,6n)).toEqual(list([q(1,2)]));expect(f(apps(history).at(-1)!,6n)).toEqual(list([q(2,3)]));expect(f(apps(cue).at(-1)!,5n)).toEqual(list([]));expect(f(apps(cue).at(-1)!,6n)).toEqual(list([q(0,1)]));
});
it('unknown history differs from neutral or known negative; absent current cue cannot reuse the last episode',async()=>{
 const unknown=await run('noHistory'),neutral=await run('neutral'),negative=await run('negative'),missing=await run('missingCue'),returned=await run('missingThenReturn');expect(f(apps(unknown).at(-1)!,5n)).toEqual(list([]));expect(f(apps(neutral).at(-1)!,5n)).toEqual(list([q(1,2)]));expect(f(apps(negative).at(-1)!,5n)).toEqual(list([q(0,1)]));expect(f(apps(missing).at(-1)!,6n)).toEqual(list([]));expect(f(apps(missing).at(-1)!,9n)).toEqual(list([]));expect(f(apps(returned)[2],6n)).toEqual(list([]));expect(f(apps(returned)[3],6n)).toEqual(list([q(0,1)]));
});
it('failed execution preserves intent and creates no conduct sample; learning140 changes only later appraisal50',async()=>{
 const base=await run(),failed=await run('failed'),visible=await run('visibleLast');for(const t of [425n,426n,1155n])expect(outputs(failed,t)).toEqual(outputs(base,t));expect(items(f(knowledge(failed,1),1n),'list')).toHaveLength(0);expect(f(apps(base)[0],5n)).toEqual(list([]));expect(f(apps(base)[1],5n)).toEqual(list([q(1,1)]));expect(apps(visible)).toEqual(apps(base));expect(estimates(knowledge(visible,1),1).disposition?.equals(estimates(knowledge(base,1),1).disposition!)).toBe(false);
});
...
it('actual target phases and contested addressed draws remain intact',async()=>{
 const s=await run('main',1,3);for(const [type,phase] of [[1153n,50n],[425n,70n],[426n,80n],[1158n,110n],[1164n,120n]] as const){const ts=s.r.snapshot().trace.map(v=>rec(v,160n)).filter(v=>records(items(f(v,13n),'list'),type).length);expect(ts).toHaveLength(type===1153n||type===1164n?8:4);for(const t of ts)expect(f(f(t,4n),3n)).toEqual(u(phase));}const d=chosenData(outputs(s,409n).at(-1)!);expect(items(f(d,9n),'list')).toHaveLength(2);for(const row of items(f(d,2n),'list'))expect(f(row,2n)).toEqual(q(1,2));expect(s.r.snapshot().randomAddresses.length).toBeGreaterThan(0);
});
