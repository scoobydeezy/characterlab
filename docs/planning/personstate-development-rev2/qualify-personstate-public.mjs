...
try {
 ...
 if(process.argv.includes('--plan')) {
  ...
  const last=(name,i=1,law=1,pressure=1,goal=1)=>apps(name,i,law,pressure,goal).at(-1);
  ...
  eq(f(last('main'),5n),list([q(2,3)]));eq(f(last('main'),6n),list([q(0,1)]));eq(f(last('main',2),5n),list([q(2,3)]));eq(f(last('main',2),6n),list([q(1,1)]));eq(f(records('main',1155n).at(-1),5n),list([true]));
  ...
  for(const t of [425n,426n,1155n]){eq(records('main',t),records('failed',t));eq(records('main',t),records('cueChanged',t));eq(records('main',t),records('main',t,1,1,2));}eq(items(f(knowledge('failed'),1n),'list').length,0);eq(f(apps('main')[0],5n),list([]));eq(f(apps('main')[1],5n),list([q(1,1)]));eq(apps('main'),apps('visibleLast'));assert.notDeepEqual(f(knowledge('main'),1n),f(knowledge('visibleLast'),1n));
  ...
  const draw=choice.chosenData(records('main',409n,1,3).at(-1));assert.equal(items(f(draw,9n),'list').length,2);for(const p of items(f(draw,2n),'list'))eq(f(p,2n),q(1,2));
