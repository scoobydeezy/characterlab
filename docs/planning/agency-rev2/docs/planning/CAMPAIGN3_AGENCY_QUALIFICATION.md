...
**Disposition: QUALIFIED / LOCAL DISPOSITION. Corrected bounded public slice COMPLETE.**
**Stage E: VER-C3-AGENCY-001. Owner decision pending: no.**
Contract: agency-public/0.2-candidate; records971..988/schema1, namespace1163.
...
## Evidence
[PlanREV2](AGENCY_PUBLIC_PLAN_REV2.json) freezes five ModelIdentities,19 complete
RunIdentity inputs, ExperimentIdentity and ComparisonCase before public qualification.
[ResultREV2](AGENCY_PUBLIC_RESULT_REV2.json) passes all19 runs and71 exact whole-prefix
continuations:52 advancing and19 terminal. Public restoration replays complete saves,
including addressed RNG and queue/state/trace/allocator equality.

[Agency tests](AGENCY_TESTS_REV2.json):36 passed, including27 public checks,8 source
...
active-suite rerun is claimed. The corrected test receipt is an actual Vitest JSON
export; the first terminal-count receipt remains preserved separately.

## Correct-forward closure
The first17-run matrix missed a later nonrecipient-view leak: a report only to A
shifted B's future occurrence IDs. Its drafted qualification and REV10 coverage were
superseded before final closure. Preserve the failed regression receipt and exact
first-cohort sources under agency-rev1/PRESERVATION.json; ResultREV1 remains a PASS
for that insufficient matrix, not closure authority.

Version0.2 gives probe-only instants the same empty source/observer reservation path
as report instants. No evidence or psychological state is manufactured. The corrected
19-run matrix includes report-to-A versus no-report followed by a later attempt;
B's full public views now match exactly. Records and namespace are unchanged.
[Closure integrity](AGENCY_CLOSURE_REV1.json) binds the final evidence and preserves
the failed cohort; run node scripts/check-agency-closure.mjs to verify it.
