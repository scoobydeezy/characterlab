...
| world event bindings / observable facets → pre-recognition `SemanticExperience` | candidate `semantic-binding/0.1-candidate` | `SEM-001A..H` accepted through `CV-SEM-090`; accepted `SEM-001I.1` fixes authoritative records/state/definitions and accepted `SEM-001I.2` freezes their permanent numeric allocation and union layouts; codecs, persistence, and integration remain unaccepted | active Campaign 1 P0 blocker; proceed to `SEM-001I.3` using only the accepted allocation table |
| `SemanticExperience` → recognition resolutions | accepted symbolic `SEM-001F,H` in `semantic-binding/0.1-candidate` | observer-owned candidate catalog, mapped identity claims, typed cues, exact unique-uncontradicted rule, immutable evaluations, append-only assert/replace/withdraw chains, and frozen phase-20/126 inputs with phase-21/127 evaluation pass `CV-SEM-061..070,087`; event/action-schema recognition remains unresolved | available within continuant-instance fixture scope; same-event belief use remains separately blocked by `ORD-001` |
...
| actual exposure/practice → embodied or procedural adaptation | none | separated from character-learning evidence | blocked pending typed adaptation-input contract |
...
The deterministic substrate is accepted prerequisite research infrastructure, not itself a psychological seam. Exact bounded scalar observation is accepted, but that result does not establish general event semantics. The active North-Star blocker is now the permanent-canonical-schema/ID and integrated-gate remainder of world-event binding truth → binding-specific observation → pre-recognition `SemanticExperience` → recognition/causal-role attachment (`SEM-001`); exact two-lane placement is accepted. Interval-aware encoding/learning remains blocked behind that shared referential grammar. A complete traversable causal slice comes later, after every edge it crosses has a versioned thin contract.
