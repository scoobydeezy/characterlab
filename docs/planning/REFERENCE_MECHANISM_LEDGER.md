...
| `MEC-004` | truth-side `EffectProvenance → perceptual projection → derived causal roles → SemanticExperience` | `PORT` with boundary correction; restricted token control now present; general port blocked by `SEM-001` | This is the strongest earned world-truth-to-character-evidence control. Preserve separately identified actor/target/instrument/cause participation, but distinguish semantic `EventRole` from analytical `CausalRole`; keep full provenance, truth referent identity, and unobserved world facets on the trace side. `SemanticExperience` receives binding-specific perceptual referents, typed observed classification, and only observer-safe evidence. Classification, track continuity, recognition, and appraisal remain separate. The current `(ConceptId, CausalRoleId, VisibleProvenanceSlotId)` path is valid only where the channel establishes identity and one winning role is sufficient. |
