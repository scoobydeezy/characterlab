...
export function personstateContext(at:bigint,pressure:number,occ:(ns:number)=>CanonicalValue){
 ...
 const w=workspaceOutput(occ(1162),ACTOR,sid(1027,'definition/personstate/agenda'),old(378,[sid(1027,'definition/personstate/prediction'),u(3),true,false]),tasks,at,{status:()=>old(372,[u(1)]),prediction:()=>undefined}).output;
 const a=appraisalOutput(occ(1163),w,tasks.map(t=>({taskKey:t.key,specId:t.specId,minimum:ZERO,maximum:ONE}))),c=concernOutput(occ(1164),a,old(385,[q(0,1),false]));
