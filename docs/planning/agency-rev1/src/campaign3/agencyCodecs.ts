/** agency-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/AGENCY_PUBLIC_ALLOCATION_TABLE.json';
...
import {commitSupportedSchemas,decodeCommit} from './commitCodecs';
...
const schemas=[...commitSupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const fail=(why:string):never=>{throw Error('AGENCY_CODEC: '+why);};
...
function validate(v:CanonicalValue):void {
 ...
 }else {decodeCommit(canonicalEncode(envelope(v)));newChildren(v);}
}
export function decodeAgency(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function agencySchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function agencyRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeAgency(canonicalEncode(record(agencySchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const agencySupportedSchemas=()=>schemas.slice();
