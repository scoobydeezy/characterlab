...
export function createAgencyRuntime(model:AgencyCompiled,input:Awaited<ReturnType<typeof compileAgencyInputs>>){
 ...
 async function handler({event,state,allocateRuntimeId}:EventHandlerContext<AuthoritativeState>){
  ...
  if(name==='context') {
   ...
   const data=chosenData(out);randomDrawRecords=items(f(data,9n),'list').map(v=>f(rec(v,422n),5n));const tie=rec(f(data,10n),423n);if(uint(f(tie,1n))===2n)randomDrawRecords.push(f(tie,3n));emit('expression',out);
  } else if(name==='expression') {
   const intent=intentOutput(occurrence(1136),event.payload),expression=expressionOutput(occurrence(1137),intent);outputs.push(intent,expression);emit('plan',expression);
  } else if(name==='plan') {
