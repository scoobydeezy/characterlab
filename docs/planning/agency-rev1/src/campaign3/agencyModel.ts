/** agency-public/0.1-candidate: finite recipes and disjoint registered owners. */
...
export const VERSION = 'agency-public/0.1-candidate';
...
export async function compileAgencyInputs(model: AgencyCompiled, initialState: Uint8Array, orderedInputs: Uint8Array, runSeed: Uint8Array) {
  ...
  for(const original of originals) {
    ...
    for(const name of ['context','probe-a','probe-b',...(kind===2n?['report']:[])]) events.push({eventId:BigInt(events.length),eventSequence:BigInt(events.length),dueAt:simInstant(at.value),phase:BigInt(STAGES.find(([n])=>n===name)![1]),eventTypeId:eventId(name),payload:original,dependencies:list([]),causalParentEventIds:[]});
  }
