...
try{
 ...
 if(process.argv.includes('--plan')){fs.writeFileSync(p+'ABSENCE_PLAN_REV1.json',JSON.stringify(plan,null,2)+'\n',{flag:'wx'});console.log('Frozen3 component models/27 runs.');}
 else{
  assert.deepEqual(JSON.parse(fs.readFileSync(p+'ABSENCE_PLAN_REV1.json')),plan);assert(!fs.existsSync(p+'ABSENCE_RESULT_REV1.json'));
  ...
  fs.writeFileSync(p+'ABSENCE_RESULT_REV1.json',JSON.stringify({status:'PASS',scope:m.VERSION,planSha256:hash(fs.readFileSync(p+'ABSENCE_PLAN_REV1.json')),models:3,runs:27,prefixes,advancing,terminal:prefixes-advancing,results},null,2)+'\n',{flag:'wx'});console.log(JSON.stringify({status:'PASS',models:3,runs:27,prefixes,advancing}));
 }
