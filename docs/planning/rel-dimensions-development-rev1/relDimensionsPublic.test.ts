...
it('derives affection without respect, respect without affection, and trust without comfort from distinct evidence',async()=>{
 ...
 expect([0,1,2,3].map(k=>probability(s,3,k))).toEqual([q(23,25),q(2,25),q(23,25),q(2,25)]);
});
...
it('observer participation and target keys govern history independently of witnessed person evidence',async()=>{
 const base=await run(),witness=await run('witnessA'),other=await run('otherTarget');expect(base.r.snapshot().state.read(path(0,0,true))).toEqual(witness.r.snapshot().state.read(path(0,0,true)));expect(witness.r.snapshot().state.read(path(0,0)).presence).toBe(false);
