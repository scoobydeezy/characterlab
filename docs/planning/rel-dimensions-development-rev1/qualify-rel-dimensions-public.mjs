...
const workerRoot='docs/planning/relDimensions-public-workers-rev1';
const p='docs/planning/',planPath=p+'REL_DIMENSIONS_PUBLIC_PLAN_REV1.json',resultPath=p+'REL_DIMENSIONS_PUBLIC_RESULT_REV1.json';
const hash=b=>createHash('sha256').update(b).digest('hex'),hex=b=>Buffer.from(b).toString('hex');
const server=await createServer({configFile:false,server:{middlewareMode:true,hmr:false},appType:'custom'});
try {
 const fx=await server.ssrLoadModule('/src/test/relDimensionsFixtures.ts'),m=await server.ssrLoadModule('/src/campaign3/relDimensionsModel.ts'),factory=await server.ssrLoadModule('/src/campaign3/relDimensionsFactory.ts'),codec=await server.ssrLoadModule('/src/campaign3/relDimensionsCodecs.ts'),data=await server.ssrLoadModule('/src/campaign2/canonicalData.ts'),ids=await server.ssrLoadModule('/src/substrate/identity.ts'),canonical=await server.ssrLoadModule('/src/substrate/canonicalEncoding.ts');
 const matrix=[...[[1,1],[2,1],[3,1],[4,1],[5,1],[1,2]].map(([law,goal])=>({law,goal,name:'main'})),...Object.keys(fx.cases()).filter(n=>n!=='main').map(name=>({law:1,goal:1,name})),{law:5,goal:1,name:'hiddenWorld'},{law:2,goal:1,name:'giftGone'},{law:3,goal:1,name:'giftGone'}];
 if(process.argv.includes('--plan')) {
  ...
  const experiment=await ids.createExperimentIdentity('corpus/0.29.0',m.VERSION,'rel-dimensions-public-harness/0.1-candidate');
  ...
  const paths=['docs/formal/REL_DIMENSIONS_PUBLIC_CONTRACT.md','docs/formal/REL_DIMENSIONS_PUBLIC_ALLOCATION_TABLE.json',p+'campaign3-rel-dimensions-model-rev1/FREEZE.json',...['Codecs','Model','Math','Runtime','Factory'].map(n=>'src/campaign3/relDimensions'+n+'.ts'),'src/test/relDimensionsFixtures.ts','src/test/relDimensionsPublic.test.ts',...['cognitiveTransforms','cognitiveMath','cognitiveChoice','cognitiveArbitration'].map(n=>'src/campaign2/'+n+'.ts'),'src/campaign3/affectFactorComparison.ts','scripts/qualify-rel-dimensions-public.mjs'];
  fs.writeFileSync(planPath,JSON.stringify({status:'FROZEN BEFORE PUBLIC QUALIFICATION',scope:'separate dyadic history, person evidence and current cue derive four relational appraisal proxies consumed by existing reason/dice response distributions; no sampled or executed action',initialState:hex(fx.initialState),runSeed:hex(fx.seed),experimentIdentity:hex(experiment.canonicalBytes),comparisonCase:hex(comparison.canonicalBytes),artifacts:paths.map(path=>({path,sha256:hash(fs.readFileSync(path))})),runs:runs.map(({identity,...r})=>r)},null,2)+'\n',{flag:'wx'});
  ...
  if(worker!==undefined){
   ...
   await Promise.all([0,1,2,3].map(async i=>{const child=await execute(process.execPath,['scripts/qualify-rel-dimensions-public.mjs','--worker='+i],{maxBuffer:1024*1024});process.stdout.write(child.stdout);if(child.stderr)process.stderr.write(child.stderr);}));
  ...
  const prob=(name,k,law=1,goal=1)=>{const p=records(name,1217n,law,goal).find(x=>key(f(f(x,2n),3n))===key(m.HOLDERS[0])&&key(f(f(x,2n),2n))===key(canonical.signed(3))&&key(f(x,3n))===key(u(k+1)));return f(items(f(p,6n),'list').find(x=>key(f(x,1n))===key(m.OPTIONS[0][k][0])),2n);};
  eq(lens(apps('main')[2]),[1,0,1,0].map(n=>list([q(n,1)])));eq(lens(apps('main')[4]),[0,1,1,1].map(n=>list([q(n,1)])));eq(lens(apps('main',1)[2]),[list([]),list([q(0,1)]),list([]),list([q(0,1)])]);eq([0,1,2,3].map(k=>prob('main',k)),[q(23,25),q(2,25),q(23,25),q(2,25)]);
  for(const [name,k] of [['giftGone',0],['taskGood',1],['broken',2],['calm',3]])for(let j=0;j<4;j++){if(j===k){assert.notDeepEqual(lens(apps(name)[2])[j],lens(apps('main')[2])[j]);assert.notDeepEqual(prob(name,j),prob('main',j));}else{eq(lens(apps(name)[2])[j],lens(apps('main')[2])[j]);eq(prob(name,j),prob('main',j));}}
  eq(lens(apps('main',0,2)[2]),[0,1,2,3].map(()=>list([q(1,2)])));eq(lens(apps('main',0,3)[2]),[0,1,2,3].map(()=>list([q(0,1)])));eq(lens(apps('giftGone',0,3)[2]),lens(apps('main',0,3)[2]));assert.notDeepEqual(lens(apps('giftGone',0,2)[2]),lens(apps('main',0,2)[2]));
  for(const law of [2,3])eq(records('main',1218n),records('main',1218n,law));
  eq(apps('main').filter(x=>key(f(x,4n))===key(m.TARGETS[0])),apps('otherTarget').filter(x=>key(f(x,4n))===key(m.TARGETS[0])));eq(f(apps('witnessA')[2],9n),f(apps('main')[2],9n));eq(f(apps('witnessA')[2],8n),list([]));eq(f(apps('witnessA')[2],10n),list([]));
  eq(f(apps('missingTask')[2],9n),list([]));eq(f(apps('missingPromise')[2],10n),list([]));eq(f(apps('missingCue')[2],11n),list([]));eq(lens(apps('main',0,4)[2]),[list([]),list([]),list([]),list([q(0,1)])]);eq(lens(apps('unknown')[2]),[list([]),list([]),list([]),list([])]);eq(prob('missingTask',1),q(1,2));
  for(const t of [1209n,1210n,1216n,1218n])eq(records('main',t),records('main',t,1,2));for(let k=0;k<4;k++)eq(prob('main',k,1,2),q(1,2));
