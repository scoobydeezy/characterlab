...
const p='docs/planning/',planPath=p+'PROCRASTINATION_PUBLIC_PLAN_REV1.json',resultPath=p+'PROCRASTINATION_PUBLIC_RESULT_REV1.json';
...
try {
 ...
 if(process.argv.includes('--plan')) {
  ...
  const paths=['docs/formal/PROCRASTINATION_PUBLIC_CONTRACT.md','docs/formal/PROCRASTINATION_PUBLIC_ALLOCATION_TABLE.json',p+'campaign3-procrastination-model-rev1/FREEZE.json',...['Codecs','Model','Math','Runtime','Factory'].map(n=>'src/campaign3/procrastination'+n+'.ts'),'src/test/procrastinationFixtures.ts','src/test/procrastinationPublic.test.ts','src/test/procrastinationRuntime.test.ts','scripts/qualify-procrastination-public.mjs'];
  ...
  fs.writeFileSync(p+'PROCRASTINATION_PUBLIC_VIEWS_REV1.json',JSON.stringify({planSha256:hash(fs.readFileSync(planPath)),views:[...views].map(([key,v])=>[key,{outputs:hex(canonical.canonicalEncode(canonical.list(v.outputs))),observerViews:v.observerViews.map(hex)}])},null,2)+'\n',{flag:'wx'});
