...
try{
 ...
 if(process.argv.includes('--sources')){const value=await sources();fs.writeFileSync(p+'CRAVING_SOURCES_REV1.json',JSON.stringify(value,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(value.packets));}
  ...
  const experiment=await ids.createExperimentIdentity('corpus/0.29.0',m.VERSION,'craving-harness/0.1-candidate'),comparison=await ids.createComparisonCase(models.map(x=>x.identity),runs.map(x=>x.identity),c.text('four laws; whole15-case roster; same observer public pressure plus fallible reports'));
  const plan={status:'FROZEN BEFORE QUALIFICATION',observer,artifacts,models:models.map(x=>({law:x.law,identity:hex(x.identity.canonicalBytes)})),experimentIdentity:hex(experiment.canonicalBytes),comparisonCase:hex(comparison.canonicalBytes),runs:runs.map(({identity,...x})=>({...x,runIdentity:hex(identity.canonicalBytes)}))};assert.equal(new Set(plan.runs.map(x=>x.runIdentity)).size,60);
  if(process.argv.includes('--plan')){fs.writeFileSync(p+'CRAVING_PLAN_REV1.json',JSON.stringify(plan,null,2)+'\n',{flag:'wx'});console.log('Frozen4 component models/60 runs.');}
  else{
   ...
   fs.writeFileSync(p+'CRAVING_RESULT_REV1.json',JSON.stringify({status:'PASS',models:4,runs:60,componentPrefixes:prefixes,advancing:240,terminal:60,publicRuns:5,publicPrefixes:source.publicPrefixes,planSha256:hash(fs.readFileSync(p+'CRAVING_PLAN_REV1.json')),results},null,2)+'\n',{flag:'wx'});console.log('PASS4 models/60 runs/300 component prefixes;5 public body runs/'+source.publicPrefixes+' public prefixes.');
  }
 }
