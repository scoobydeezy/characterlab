...
export function cases(){const main:Frame[]=[{at:1},{at:2,condition:2},{at:3,condition:3,catalogue:2},{at:4,condition:0,catalogue:2}];
 return {main,hiddenTruth:main.map(x=>({...x,truth:false})),calm:main.map(x=>({...x,display:false})),reserve:main.map(x=>({...x,reserve:2})),control:main.map(x=>({...x,display:x.condition===3?false:true})),
