/** affect-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/AFFECT_PUBLIC_ALLOCATION_TABLE.json';
...
import {beliefSupportedSchemas,decodeBelief} from './beliefCodecs';
...
const schemas=[...beliefSupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const fail=(why:string):never=>{throw Error('AFFECT_CODEC: '+why);};
...
function validate(v:CanonicalValue):void {
 ...
 }else {decodeBelief(canonicalEncode(envelope(v)));newChildren(v);}
}
export function decodeAffect(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function affectSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function affectRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeAffect(canonicalEncode(record(affectSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const affectSupportedSchemas=()=>schemas.slice();
