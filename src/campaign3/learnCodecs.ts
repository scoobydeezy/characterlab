/** learn-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/LEARN_PUBLIC_ALLOCATION_TABLE.json';
...
const fail=(why:string):never=>{throw Error('LEARN_CODEC: '+why);};
...
export function decodeLearn(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function learnSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function learnRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeLearn(canonicalEncode(record(learnSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const learnSupportedSchemas=()=>schemas.slice();
