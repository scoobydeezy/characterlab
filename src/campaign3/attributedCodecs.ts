/** attributed-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/ATTRIBUTED_PUBLIC_ALLOCATION_TABLE.json';
...
const fail=(why:string):never=>{throw Error('ATTRIBUTED_CODEC: '+why);};
...
export function decodeAttributed(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function attributedSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function attributedRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeAttributed(canonicalEncode(record(attributedSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const attributedSupportedSchemas=()=>schemas.slice();
