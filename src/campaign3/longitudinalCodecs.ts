/** longitudinal-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/LONGITUDINAL_PUBLIC_ALLOCATION_TABLE.json';
...
import {relationshipSupportedSchemas,decodeRelationship} from './relationshipCodecs';
...
const schemas=[...relationshipSupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const fail=(why:string):never=>{throw Error('LONGITUDINAL_CODEC: '+why);};
...
function validate(v:CanonicalValue):void {
 ...
 }else {decodeRelationship(canonicalEncode(envelope(v)));newChildren(v);}
}
export function decodeLongitudinal(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function longitudinalSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function longitudinalRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeLongitudinal(canonicalEncode(record(longitudinalSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const longitudinalSupportedSchemas=()=>schemas.slice();
