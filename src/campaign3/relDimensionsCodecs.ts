/** relDimensions-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/REL_DIMENSIONS_PUBLIC_ALLOCATION_TABLE.json';
...
const fail=(why:string):never=>{throw Error('REL_DIMENSIONS_CODEC: '+why);};
...
export function decodeRelDimensions(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function relDimensionsSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function relDimensionsRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeRelDimensions(canonicalEncode(record(relDimensionsSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const relDimensionsSupportedSchemas=()=>schemas.slice();
