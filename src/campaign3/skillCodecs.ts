/** skill-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/SKILL_PUBLIC_ALLOCATION_TABLE.json';
...
import {workSupportedSchemas,decodeWork} from './workCodecs';
...
const schemas=[...workSupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const fail=(why:string):never=>{throw Error('SKILL_CODEC: '+why);};
...
function validate(v:CanonicalValue):void {
 ...
 }else {decodeWork(canonicalEncode(envelope(v)));newChildren(v);}
}
export function decodeSkill(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function skillSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function skillRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeSkill(canonicalEncode(record(skillSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const skillSupportedSchemas=()=>schemas.slice();
