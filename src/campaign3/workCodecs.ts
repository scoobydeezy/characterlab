/** workspace-control-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/WORKSPACE_CONTROL_ALLOCATION_TABLE.json';
...
import {affectSupportedSchemas,decodeAffect} from './affectCodecs';
...
const schemas=[...affectSupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const fail=(why:string):never=>{throw Error('WORK_CODEC: '+why);};
...
function validate(v:CanonicalValue):void {
 ...
 }else {decodeAffect(canonicalEncode(envelope(v)));newChildren(v);}
}
export function decodeWork(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function workSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function workRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeWork(canonicalEncode(record(workSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const workSupportedSchemas=()=>schemas.slice();
