/** goal-strategy-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/GOAL_STRATEGY_PUBLIC_ALLOCATION_TABLE.json';
...
import {agencySupportedSchemas,decodeAgency} from './agencyCodecs';
...
const schemas=[...agencySupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const fail=(why:string):never=>{throw Error('GOAL_CODEC: '+why);};
...
function validate(v:CanonicalValue):void {
 ...
 }else {decodeAgency(canonicalEncode(envelope(v)));newChildren(v);}
}
export function decodeGoalStrategy(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function goalStrategySchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function goalStrategyRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeGoalStrategy(canonicalEncode(record(goalStrategySchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const goalStrategySupportedSchemas=()=>schemas.slice();
