/** reason-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/REASON_PUBLIC_ALLOCATION_TABLE.json';
...
const fail=(why:string):never=>{throw Error('REASON_CODEC: '+why);};
...
export function decodeReason(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function reasonSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function reasonRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeReason(canonicalEncode(record(reasonSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const reasonSupportedSchemas=()=>schemas.slice();
