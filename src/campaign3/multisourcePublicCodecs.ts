/** multisource-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/MULTISOURCE_PUBLIC_ALLOCATION_TABLE.json';
...
import {generalAttentionSupportedSchemas,decodeGeneralAttention} from './generalAttentionCodecs';
import {validateSemanticReferent} from '../substrate/referentOrigin';
type R=Extract<CanonicalValue,{kind:'record'}>;
type T=string|{list:T;min:number;max:number}|{map:T[];min:number;max:number}|{enum:number[]}|{oneOf:T[]};
...
const schemas=[...generalAttentionSupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const context={admittedVersions:['local-reserve-level-observation/0.1-candidate','body-formation-evidence-component/0.1-candidate','trial-panel-source-component/0.1-candidate']};
const fail=(why:string):never=>{throw Error('MULTISOURCE_CODEC: '+why);};
...
function validate(v:CanonicalValue):void {
 ...
 if(typeof v!=='boolean'&&v.kind==='record'&&byId.has(v.schema.typeId)){
  ...
  const union=allocation.unions.find(u=>BigInt(u.typeId)===v.schema.typeId);
  if(union){const tag=v.fields.get(BigInt(union.tagField));if(typeof tag==='boolean'||tag?.kind!=='unsigned')fail('tag');const variant=union.variants.find(x=>BigInt(x.tag)===(tag as {value:bigint}).value);if(!variant||variant.required.some(n=>!v.fields.has(BigInt(n)))||variant.forbidden.some(n=>v.fields.has(BigInt(n))))fail('union fields');}
 }else {decodeGeneralAttention(canonicalEncode(envelope(v)),context);newChildren(v);}
}
export function decodeMultisource(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function multisourceSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function multisourceRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeMultisource(canonicalEncode(record(multisourceSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const multisourceSupportedSchemas=()=>schemas.slice();
