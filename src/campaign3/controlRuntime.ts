...
const r=(n:number,v:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>)=>n>=1017?cr(n,v):habitRecord(n,v);
...
import {controlSummary,controlOptions,controlRaw,controlReasons,controlDecision,controlObserve,appendControl} from './controlMath';
export function createControlRuntime(model:ControlCompiled,input:Awaited<ReturnType<typeof compileControlInputs>>){
 ...
 async function handler({event,state,allocateRuntimeId}:EventHandlerContext<AuthoritativeState>){
  ...
  if(name==='appraise'){
   ...
   const summary=controlSummary({candidate:1,law:model.settings.law},journal),fields=new Map<bigint,CanonicalValue>([[1n,occurrence()],[2n,f(src,1n)],[3n,f(src,6n)],[4n,f(src,2n)],[5n,model.settings.candidate===6?qValue(ZERO):f(rec(summary,824n),f(src,6n)===true?2n:1n)]]);if(belief)fields.set(6n,belief);
   ...
   const base=rec(controlRaw(event.payload,occurrence()),1017n),o=rec(event.payload,827n),app=rec(f(o,2n),826n),c=rec(controls.get(event.dueAt)!.evaluation,1015n),signals=[...items(f(base,3n),'list')];
   if(uint(f(app,4n))===2n&&f(c,3n)===true)signals.push(old(402,[old(401,[OPTIONS[1],TASK,u(1)]),qValue(ONE),old(400,[map([])])]));output=r(1017,[f(base,1n),event.payload,list(signals)]);
  }
  else if(name==='reasons')output=controlReasons(model,event.payload,occurrence());
  else if(name==='decision'){output=await controlDecision(event.payload,event.dueAt,occurrence(),input.runSeed);randomDrawRecords=[...items(f(rec(output,1019n),6n),'list')];}
  else if(name==='intent')output=r(1020,[occurrence(),event.payload]);
  else if(name==='expression'){const chosen=f(rec(f(rec(event.payload,1020n),2n),1019n),7n);output=r(1021,[occurrence(),event.payload,list(OPTIONS.map(o=>qValue(choiceAlignment(chosen,new Map(OPTIONS.map(x=>[key(x),key(x)===key(o)?ONE:ZERO]))))))]);}
  else if(name==='plan'){const decision=rec(f(rec(f(rec(event.payload,1021n),2n),1020n),2n),1019n);output=r(1022,[occurrence(),event.payload,f(rec(f(decision,7n),395n),2n)]);}
  else if(name==='attempt')output=r(1023,[occurrence(),event.payload]);
  else if(name==='execute'){const plan=rec(f(rec(event.payload,1023n),2n),1022n),src=rec(f(rec(physical.get(event.dueAt)!,1010n),1n),818n),performed=key(f(plan,3n))===key(f(rec(OPTIONS[0],395n),2n));output=r(1024,[occurrence(),event.payload,performed,f(src,3n)]);}
  else if(name==='observe'){output=controlObserve(event.payload,f(rec(physical.get(event.dueAt)!,1010n),1n),occurrence());for(const n of ['history','belief','cache'])emit(n,output);}
  else if(name==='history'){const prior=read(historyPath)!,result=model.settings.candidate===5&&f(rec(controls.get(event.dueAt)!.evaluation,1015n),5n)===true?{next:r(820,[list([])]),applied:true}:appendControl(prior,event.payload);output=r(837,[occurrence(),event.payload,result.applied,prior,result.next]);if(result.applied)write(historyPath,prior,result.next);}
