/** dependence-substitutes-component/0.2-candidate; run-owned component, no public authority. */
...
export const VERSION='dependence-substitutes-component/0.2-candidate';
...
export const OPTIONS=['A','B'].map(n=>r(395,[ACTOR,sid(1027,'action/substitution/'+n)]));
 ...
 return Object.freeze({snapshot,save,observerView:()=>clone({journal,belief,rows}),
  async step(fault?:'after-decision'|'before-commit'){
   ...
   try{
    ...
    const indices=admitted.flatMap((v,i)=>v?[i]:[]);
