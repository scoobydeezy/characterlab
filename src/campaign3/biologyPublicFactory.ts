...
import {compileBiologyModel,compileBiologyInputs,copyData,STAGES,type BiologyCompiled,type BiologySource} from './biologyPublicModel';
...
export async function restoreBiologyPublicRun(source:BiologySource,input:{initialState:Uint8Array;orderedInputs:Uint8Array;save:Uint8Array}){
 try{const original=copyData(input,['initialState','orderedInputs','save']),saved=rec(decode(original.save),132n),identity=rec(f(saved,3n),104n),seed=f(identity,4n);if(typeof seed==='boolean'||seed.kind!=='bytes')throw Error('BIOLOGY_SAVE_SEED');const run=await createBiologyPublicRun(await prepareBiologyModel(source),{initialState:original.initialState,orderedInputs:original.orderedInputs,runSeed:seed.value});if(key(decode(run.runIdentity()))!==key(identity))throw Error('BIOLOGY_SAVE_IDENTITY');const count=items(f(saved,11n),'list').length;if(count%STAGES.length)throw Error('BIOLOGY_SAVE_PARTIAL_INSTANT');for(let i=0;i<count/STAGES.length;i++)if(!await run.settleNextInstant())throw Error('BIOLOGY_SAVE_PREFIX');const actualSave=run.save();if(actualSave.length!==original.save.length||!actualSave.every((b,i)=>b===original.save[i]))throw Error('BIOLOGY_SAVE_WHOLE_EQUALITY');return run;}catch(e){throw new SaveContractError(e instanceof Error?e.message:String(e));}
}
