/** social-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/SOCIAL_PUBLIC_ALLOCATION_TABLE.json';
...
import {skillSupportedSchemas,decodeSkill} from './skillCodecs';
...
const schemas=[...skillSupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const fail=(why:string):never=>{throw Error('SOCIAL_CODEC: '+why);};
...
function validate(v:CanonicalValue):void {
 ...
 }else {decodeSkill(canonicalEncode(envelope(v)));newChildren(v);}
}
export function decodeSocial(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function socialSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function socialRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeSocial(canonicalEncode(record(socialSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const socialSupportedSchemas=()=>schemas.slice();
