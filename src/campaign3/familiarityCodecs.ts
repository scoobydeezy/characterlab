/** familiarity-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/FAMILIARITY_PUBLIC_ALLOCATION_TABLE.json';
...
const fail=(why:string):never=>{throw Error('FAMILIARITY_CODEC: '+why);};
...
export function decodeFamiliarity(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function familiaritySchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function familiarityRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeFamiliarity(canonicalEncode(record(familiaritySchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const familiaritySupportedSchemas=()=>schemas.slice();
