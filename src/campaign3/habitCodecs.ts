/** habit-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/HABIT_PUBLIC_ALLOCATION_TABLE.json';
...
import {socialSupportedSchemas,decodeSocial} from './socialCodecs';
...
const schemas=[...socialSupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const fail=(why:string):never=>{throw Error('HABIT_CODEC: '+why);};
...
function validate(v:CanonicalValue):void {
 ...
 }else {decodeSocial(canonicalEncode(envelope(v)));newChildren(v);}
}
export function decodeHabit(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function habitSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function habitRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeHabit(canonicalEncode(record(habitSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const habitSupportedSchemas=()=>schemas.slice();
