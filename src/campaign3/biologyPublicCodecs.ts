/** biology-public-allocation/0.1-candidate. Closed structural grammar only. */
import allocation from '../../docs/formal/BIOLOGY_PUBLIC_ALLOCATION_TABLE.json';
...
import {habitSupportedSchemas,decodeHabit} from './habitCodecs';
...
const schemas=[...habitSupportedSchemas(),...added],registry=new RecordSchemaRegistry(schemas),byId=new Map(allocation.records.map(d=>[BigInt(d.typeId),d]));
const fail=(why:string):never=>{throw Error('BIOLOGY_PUBLIC_CODEC: '+why);};
...
function typed(v:CanonicalValue,t:T):void {
 ...
 if(t==='data'){validateData(v);return;}
...
function validate(v:CanonicalValue):void {
 ...
 if(typeof v!=='boolean'&&v.kind==='record'&&byId.has(v.schema.typeId)){
  ...
  if(v.schema.typeId===1412n){const field=(id:bigint)=>v.fields.get(id) as {value:string|bigint},option=String(field(4n).value),domain=String(field(5n).value),kind=field(3n).value,strength=BigInt(field(6n).value);const domains=['pleasure','withdrawal','hunger','thirst','sleep','pain','stress','protection','practice','protective-goal','work-goal'];if(!['drug','meal','drink','sleep','relax','work','alternative','withhold'].includes(option)||!domains.includes(domain)||strength===0n||strength>1000n||strength< -1000n||kind!==BigInt(domain==='pleasure'?2:['practice','protective-goal','work-goal'].includes(domain)?3:domain==='protection'?4:1))fail('source role');}

 }else {decodeHabit(canonicalEncode(envelope(v)));newChildren(v);}
}
export function decodeBiologyPublic(bytes:Uint8Array):CanonicalValue {const value=canonicalDecode(bytes,registry);validate(value);return value;}
export function biologyPublicSchema(name:string|number):RecordSchema {return added.find(s=>typeof name==='number'?s.typeId===BigInt(name):s.name===name)??fail('schema');}
export function biologyPublicRecord(name:string|number,values:readonly CanonicalValue[]|ReadonlyMap<bigint,CanonicalValue>):R {return decodeBiologyPublic(canonicalEncode(record(biologyPublicSchema(name),values instanceof Map?values:new Map((values as readonly CanonicalValue[]).map((v,i)=>[BigInt(i+1),v]))))) as R;}
export const biologyPublicSupportedSchemas=()=>schemas.slice();

export function validateData(v:CanonicalValue):void {
 if(typeof v==='boolean')return;
 if(v.kind==='signed'){if(v.value>BigInt(Number.MAX_SAFE_INTEGER)||v.value<BigInt(Number.MIN_SAFE_INTEGER))throw Error('BIOLOGY_DATA_INTEGER');return;}
 if(v.kind==='text')return;
 if(v.kind==='set'&&!v.items.length)return;
 if(v.kind==='list'){v.items.forEach(validateData);return;}
 if(v.kind==='map'){for(const [k,x] of v.entries){if(typeof k==='boolean'||k.kind!=='text'||['__proto__','constructor','prototype'].includes(k.value))throw Error('BIOLOGY_DATA_KEY');validateData(x);}return;}
 throw Error('BIOLOGY_DATA_KIND');
}
