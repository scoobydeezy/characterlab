...
export const TASKS=['informing','avoid-repetition'].map(n=>old(371,[ACTOR,semanticReferentFromAuthoredContent(sid(1038,'content/attributed/'+n))]));
export const OPTIONS=['explain','defer'].map(n=>old(395,[ACTOR,sid(1027,'action/attributed/'+n)]));
...
export const path=(i:number):StatePath=>({rootStateTypeId:1135n,fieldId:1n,selectors:[{kind:'mapKey',key:HOLDERS[i]}]});
...
const ownership=()=>compileMutationAuthorityRegistry([0,1,2].map(i=>({authorityName:'authority/attributed-holder-'+i,ownedLeaves:[{pattern:pattern(path(i)),valueGrammar:{kind:'canonical-record' as const,recordTypeId:1134n},removalAllowed:false}]})));
...
export async function compileAttributedModel(input:AttributedSource){
 const source=copyData(input,['parameters','content','registry']),profile=rec(decode(source.parameters),1131n),law=Number(uint(f(profile,2n))),pressure=Number(uint(f(profile,3n))),recipe=attributedRecipe(law,pressure);for(const n of ['parameters','content','registry'] as const)if(key(decode(source[n]))!==key(decode(recipe[n])))throw Error('ATTRIBUTED_EXACT_MODEL');
 const own=ownership(),authority=new StateAuthorityRegistry(own.writableLeaves,own.authorities),content=rec(decode(source.content),1145n),modelIdentity=await createModelIdentity({rulesVersion:VERSION,contentSchemaVersion:VERSION,contentManifest:await commitManifest(content),parameterSchemaVersion:VERSION,parameterSet:await commitManifest(profile),numericProfileVersion:'attributed-exact/0.1-candidate',randomAlgorithmVersion:RANDOM_ALGORITHM_VERSION,registrySchemaVersion:VERSION,registryManifest:await commitManifest(decode(source.registry))});
...
export async function compileAttributedInputs(model:AttributedCompiled,initialState:Uint8Array,orderedInputs:Uint8Array,runSeed:Uint8Array){
 ...
 const events:ScheduledEvent[]=originals.map((v,i)=>{const o=rec(v,1132n),at=instant(f(o,1n));if(at<=last)throw Error('ATTRIBUTED_INPUT_ORDER');last=at;return {eventId:BigInt(i),eventSequence:BigInt(i),dueAt:simInstant(BigInt(at)),phase:0n,eventTypeId:eventId('source'),payload:v,dependencies:list([]),causalParentEventIds:[]};});
