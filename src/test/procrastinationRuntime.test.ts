...
it('rejects adoption without a pre-deadline work window',async()=>{const m=await compileProcrastinationModel(procrastinationRecipe());for(const at of [6,7,8])await expect(compileProcrastinationInputs(m,enc(m.initial.canonicalValue()),procrastinationInputs([{at,instruction:1}]),seed)).rejects.toThrow('ADOPTION_WINDOW');});
