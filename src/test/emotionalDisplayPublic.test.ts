...
it('keeps reassurance fixed while distress changes, and distress fixed while chosen content changes',async()=>{
 ...
 for(const s of [a,calm])for(const v of outputs(s,1105n))expect(f(rec(v,1105n),5n)).toEqual(list([false]));expect(f(rec(outputs(disclose,1105n)[0],1105n),5n)).toEqual(list([true]));expect(f(rec(outputs(a,1108n)[0],1108n),6n)).toEqual(list([q(1,1)]));
});
