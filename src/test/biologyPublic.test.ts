...
import {copyData} from '../campaign3/biologyPublicBytes';
...
it('copies large native saves through intrinsic bytes without invoking host metadata',()=>{const save=new Uint8Array(20_000_000);save[save.length-1]=231;let called=false;Object.defineProperty(save,'metadata',{get(){called=true;throw Error('must not execute');}});const copied=copyData({save},['save']).save;expect(copied.length).toBe(save.length);expect(copied.at(-1)).toBe(231);expect(called).toBe(false);save[0]=4;expect(copied[0]).toBe(0);expect(()=>copyData({save:new Proxy(save,{})},['save'])).toThrow();});
