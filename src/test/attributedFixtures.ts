...
export function cases(){
 ...
 return {reportChangeFixed:main.map(x=>({...x,report:2,delivery:false})),main,hiddenTruth:main.map(x=>({...x,truth:false})),hiddenTarget:main.map(x=>({...x,target:true})),targetAbsent:main.map(x=>({...x,targetAccess:false})),
