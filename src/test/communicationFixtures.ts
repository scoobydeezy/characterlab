...
import {communicationRecord as r} from '../campaign3/communicationCodecs';
...
export const original=(x:Frame)=>r(1069,[signed(x.at),x.truth??true,x.display??true,x.privateAccess??true,x.delivery??true,x.a??true,x.b??true]);
...
export function cases(){
 ...
 return {main,hiddenTruth:main.map(x=>({...x,truth:false})),concealedChange:main.map(x=>({...x,display:!(x.display??true)})),
