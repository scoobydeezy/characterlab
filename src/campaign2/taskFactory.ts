/** Frozen measurement-task run creation. Restore is separately prefix-validated. */
...
import {compileTaskModel} from './taskModel';
...
declare const taskModelBrand:unique symbol;
export interface TaskModel {readonly [taskModelBrand]:true;}
const models=new WeakMap<object,Awaited<ReturnType<typeof compileTaskModel>>>();
function taskModelFacts(handle:TaskModel){const m=models.get(handle);if(!m)invalidModel('prepared task model required');return m;}
export async function prepareTaskModel(source:Campaign2ModelSource):Promise<TaskModel>{const m=await compileTaskModel(source),handle=Object.freeze({}) as TaskModel;models.set(handle,m);return handle;}
export function taskModelIdentity(handle:TaskModel):Uint8Array{return taskModelFacts(handle).modelIdentity.canonicalBytes.slice();}
...
function expose(runtime:Runtime,model:ReturnType<typeof taskModelFacts>,inputs:Inputs){
...
export async function createTaskRun(handle:TaskModel,input:{initialState:Uint8Array;orderedInputs:Uint8Array;runSeed:Uint8Array}){
 const model=taskModelFacts(handle),data=runData(input),state=model.stateModel.restoreState(data.initialState);
 ...
 for(const e of state.entries())if(e.path.rootStateTypeId===373n&&key(f(rec(e.value,372n),1n))!==key({kind:'unsigned',value:1n}))invalidModel('initial tasks must be Open');
 const base=model.base;base.domains.validateStatic(state);base.domains.validateReferences(state,0n);
 const inputs=await compileOrderedInputProfile(model.profiles.orderedInput,model.content,base.domains,decodeTask).create(data.orderedInputs,data.initialState,model.modelIdentity,data.runSeed);
 const evaluator=compileAdaptationEvaluator(base.adaptation,base.domains,model.stateModel);
 const runtime=createAdaptationRuntime(inputs,state,base.admission,evaluator,base.domains,model.stateModel,base.parameters.maxWork,base.bridge,undefined,base.probe,base.measurement,model.predictionComponent.memoryComponent,[],model.predictionComponent,[],model);
 return expose(runtime,model,inputs);
}
export async function restoreTaskRun(source:Campaign2ModelSource,input:{initialState:Uint8Array;orderedInputs:Uint8Array;save:Uint8Array}){
 ...
 const savedBytes=copies.runSeed,save=rec(decodeTask(savedBytes),132n),runIdentity=await restoreRunIdentity(f(save,3n));
 ...
 const handle=await prepareTaskModel(source),model=taskModelFacts(handle),base=model.base;
 const replay=await createTaskRun(handle,{initialState:copies.initialState,orderedInputs:copies.orderedInputs,runSeed:seed.value});
 if(key(decodeTask(replay.runIdentity()))!==key(runIdentity.value))throw new SaveContractError('original S0/input/model commitments differ');
 ...
 let count=0;while(count<N){try{if(!await replay.settleNextInstant())throw new SaveContractError('prefix exhausted');}catch(error){throw new SaveContractError('prefix execution failed: '+(error instanceof Error?error.message:String(error)));}
  count=items(decodeTask(replay.snapshot().trace),'list').length;if(count>N)throw new SaveContractError('prefix crosses whole-instant boundary');
 }
 if(key(decodeTask(replay.save()))!==key(save))throw new SaveContractError('whole-save prefix equality failed');
 const facts=privateRuns.get(replay)!.taskPendingFacts(),memoryFacts=privateRuns.get(replay)!.memoryPendingFacts(),predictionFacts=privateRuns.get(replay)!.predictionPendingFacts(),allFacts=[...facts,...memoryFacts,...predictionFacts]; // Detached facts only, after exact equality.
 ...
 const prepared=await prepareCanonicalSave(savedBytes,{stateAdapter:adapter,eventTypeKeys:eventKeys,maxSettlementWorkPerSimulationInstant:base.parameters.maxWork,expectedModelIdentity:model.modelIdentity,expectedRunIdentity:runIdentity,additionalSchemas:taskSupportedSchemas().filter(s=>![...Object.values(identitySchemas),...Object.values(persistenceSchemas)].some(b=>b.typeId===s.typeId))});
 ...
 const profile=compileOrderedInputProfile(model.profiles.orderedInput,model.content,base.domains,decodeTask);
 ...
 const evaluator=compileAdaptationEvaluator(base.adaptation,base.domains,model.stateModel);
 const runtime=createAdaptationRuntime(inputs,prepared.state,base.admission,evaluator,base.domains,model.stateModel,base.parameters.maxWork,base.bridge,prepared,base.probe,base.measurement,model.predictionComponent.memoryComponent,memoryFacts,model.predictionComponent,predictionFacts,model,facts);
 return expose(runtime,model,inputs);
}
