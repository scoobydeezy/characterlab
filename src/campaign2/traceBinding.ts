...
export function compilePredictionBaseTraceBinding(modelIdentity:CanonicalValue,runIdentity:CanonicalValue,probe:ReturnType<typeof compileProbeExecution>){return compileBinding(modelIdentity,runIdentity,probe,true,true,true);}
export function compileTaskBaseTraceBinding(modelIdentity:CanonicalValue,runIdentity:CanonicalValue,probe:ReturnType<typeof compileProbeExecution>){return compileBinding(modelIdentity,runIdentity,probe,true,true,true,true);}
export function compileCognitiveBaseTraceBinding(modelIdentity:CanonicalValue,runIdentity:CanonicalValue,probe:ReturnType<typeof compileProbeExecution>){return compileBinding(modelIdentity,runIdentity,probe,true,true,true,true,true);}
function compileBinding(modelIdentity:CanonicalValue,runIdentity:CanonicalValue,probe?:ReturnType<typeof compileProbeExecution>,measurement=false,memory=false,prediction=false,task=false,cognitive=false){
 const model=rec(modelIdentity,103n),run=rec(runIdentity,104n);
 if(!supportsTraceProfile(TRACE_PROFILE)||txt(f(model,1n))!==(cognitive?'rules/campaign2-task-cognitive/0.1-candidate':task?'rules/campaign2-task-commitment/0.1-candidate':prediction?'rules/campaign2-measurement-prediction/0.1-candidate':memory?'rules/campaign2-measurement-memory/0.1-candidate':measurement?'rules/campaign2-measurement-evidence/0.1-candidate':probe?'rules/campaign2-regulatory-probe/0.2-candidate':TRACE_RULES)||key(f(run,1n))!==key(model))throw new SchedulerContractError('INVALID_CONFIGURATION','canonical trace requires exact 0.2 model/run binding');
