...
const output='docs/planning/TASK_EARNED_FEEDBACK_CALIBRATION_REV1.json';
assert(!fs.existsSync(output),'preserve prior report');
const paths=['reference/src/model/decision.ts','reference/src/model/diceCompiler.ts','reference/src/model/identity.ts','reference/src/kernel/evidenceOverlap.ts','scripts/explore-task-earned-feedback.mjs'];
...
try{
 ...
 const compile=(m,state)=>compileReasonDice(new Map(['A','B'].map((o,i)=>{
  const pulls=identityFeedbackRawInfluences(o,semantic(m),new Map([['CommitmentFidelity',state]]),polarity,K);
 ...
 fs.writeFileSync(output,JSON.stringify({status:'EXECUTED HISTORICAL RESEARCH; NOT CANONICAL IDENTITY QUALIFICATION',sourceFingerprints:before,parameters:{base:'1/10',gain:'1/1',thetaReason:'37/100',identityK:'1/10',thetaRoll:'1/2',thetaPlayer:'1/2',modifierUnit:'1/1'},cases,shiftedCases:shifted.length,limitations:['All three choices and identity updates use the actual historical components, including quantization and alignment-scaled feedback.','The semantic map from task concern to commitment meaning and expression qualification are proposed operands; DEC-001 is not closed.','Later means are derived hypothetical protocol outcomes, not executed generated actions or current prediction evidence.','Source/evidence strings are research-only controls, never canonical provenance.','The sampled seeds do not prove universal boundary movement. A profile requiring a witness must name its committed seed and actual generated path.','Identity is initially empty here; the earlier raw standing1/3 intervention was not the historical feedback projection.']},null,2)+'\n');
