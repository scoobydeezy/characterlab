...
const output='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV9.json';assert(!fs.existsSync(output));
...
fs.writeFileSync(output,JSON.stringify({version:'ga-identity-role-closure/0.9-draft',status:'SYMBOLIC ROLE CROSSWALK; PUBLIC COMPILER AND ALLOCATION OPEN',recordCount:records.length,positions,boundaries,stateMapKeyRoles,stateMapKeyDisposition:'Symbolic six scalar new-root key roles plus composite physical key recursion; actual compiler and inherited root coverage remain separate.',sources:[...inputs,'docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV9.md','scripts/build-ga-identity-role-closure-rev9.mjs'].map(fp)},null,2)+'\n');
