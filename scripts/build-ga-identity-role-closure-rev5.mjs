...
const output='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV5.json';assert(!fs.existsSync(output));
...
const packets=inputs.map(p=>JSON.parse(fs.readFileSync(p))),records=packets.flatMap(p=>p.records),names=new Set(records.map(r=>r.name));assert.equal(names.size,116);
