...
const source='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV9.json',output='docs/planning/GA_IDENTITY_ROLE_REVIEW_REV9.json';assert(!fs.existsSync(output));const original=JSON.parse(fs.readFileSync(source));
function validate(p){
 assert.equal(p.recordCount,129);assert.equal(p.positions.length,128);assert.equal(p.boundaries.length,186);assert.equal(new Set(p.positions.map(x=>x.path)).size,128);
