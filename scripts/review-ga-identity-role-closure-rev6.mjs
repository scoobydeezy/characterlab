...
const source='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV6.json',output='docs/planning/GA_IDENTITY_ROLE_REVIEW_REV6.json';assert(!fs.existsSync(output));const original=JSON.parse(fs.readFileSync(source));
function validate(p){
 assert.equal(p.recordCount,124);assert.equal(p.positions.length,122);assert.equal(p.boundaries.length,168);assert.equal(new Set(p.positions.map(x=>x.path)).size,122);
