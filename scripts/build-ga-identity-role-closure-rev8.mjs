...
const output='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV8.json';assert(!fs.existsSync(output));
const inputs=['GA_SURVIVING_CHILD_CARRIER_REV3','GA_GOAL_RESULT_CARRIER_REV2','GA_FORMATION_PROTOCOL_CARRIER_REV2','GA_RECOLLECTION_CARRIER_REV3','GA_BODY_PIPELINE_CARRIER_REV3','GA_VISUAL_PIPELINE_CARRIER_REV2','GA_STATE_ROOT_KEY_INVENTORY_REV2','GA_PRIOR_FEEDBACK_CARRIER_REV1','GA_LOCAL_RESERVE_PUBLIC_CARRIER_REV1','GA_REQUESTED_SOURCE_CARRIER_REV1','GA_SOURCE_PROFILE_DECLARATIONS_REV1','GA_SOURCE_STAGE_CLOSURE_REV1'].map(n=>'docs/planning/'+n+'.json');
const packets=inputs.map(p=>JSON.parse(fs.readFileSync(p))),records=packets.flatMap(p=>p.records),names=new Set(records.map(r=>r.name));assert.equal(names.size,129);
...
fs.writeFileSync(output,JSON.stringify({version:'ga-identity-role-closure/0.8-draft',status:'SYMBOLIC ROLE CROSSWALK; PUBLIC COMPILER AND ALLOCATION OPEN',recordCount:records.length,positions,boundaries,stateMapKeyRoles,stateMapKeyDisposition:'Symbolic six scalar new-root key roles plus composite physical key recursion; actual compiler and inherited root coverage remain separate.',sources:[...inputs,'docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV8.md','scripts/build-ga-identity-role-closure-rev8.mjs'].map(fp)},null,2)+'\n');
