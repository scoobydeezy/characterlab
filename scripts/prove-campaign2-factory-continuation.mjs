...
if(process.argv[2]?.startsWith('--worker-')){
  ...
  try{
    ...
    if(process.argv[2]==='--worker-create'){
      ...
      const source=firstModelCandidate(),model=await factory.prepareCampaign2Model(source);
      ...
      await run.settleNextInstant();put('expected.hex',hex(run.save()));
  ...
  try{
    ...
    const report={status:'PASS',control:'FCT-5 fresh-process continuation',processes:2,originalInitialStateTransferred:false,
      checkpointRoundTrip:'byte-identical',continuedFullSave:'byte-identical',finalSaveBytes:actual.length/2,
      finalSaveSha256:crypto.createHash('sha256').update(Buffer.from(actual,'hex')).digest('hex'),
      scope:'Restricted facade continuation only; canonical transition trace completion and independent release qualification remain separate.'};
    fs.writeFileSync(path.join(root,'docs/planning/CAMPAIGN2_FACTORY_CONTINUATION_PROOF.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));
  }finally{
