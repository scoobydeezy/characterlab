// MODEL-BYTE-A: actual deletion + fresh-process reconstruction; no derived input reaches child.
...
const folder=path.resolve(root,'docs/planning/campaign2-first-model');
...
try{
  ...
  execFileSync(process.execPath,[path.join(root,'scripts/materialize-campaign2-model.mjs'),'--write'],{cwd:root,stdio:'pipe',windowsHide:true});
  targets.forEach((p,i)=>assert.deepEqual(fs.readFileSync(p),before[i],files[i]));
  passed=true;
  const report={control:'MODEL-BYTE-A',status:'PASS',deletedArtifacts:files,freshProcess:true,
    childInputs:'source declarations and exact version/profile constants only; no stored derived artifacts',
    comparisons:files.map((name,i)=>({name,sha256:crypto.createHash('sha256').update(before[i]).digest('hex'),result:'byte-identical'}))};
  fs.writeFileSync(path.join(root,'docs/planning/CAMPAIGN2_MODEL_BYTE_A.json'),JSON.stringify(report,null,2)+'\n');
  console.log('MODEL-BYTE-A PASS: 15 artifacts deleted and reproduced byte-identically in a fresh process.');
}finally{
