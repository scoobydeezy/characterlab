...
const planPath=p+'REL_DIMENSIONS_PUBLIC_PLAN_REV1.json',resultPath=p+'REL_DIMENSIONS_PUBLIC_RESULT_REV1.json';
const plan=read(planPath),result=read(resultPath),tests=read(p+'REL_DIMENSIONS_TESTS_REV2.json'),reference=read(p+'REL_DIMENSIONS_REFERENCE_TESTS_REV1.json');
assert.equal(result.status,'PASS');assert.equal(result.planSha256,sha(planPath));
assert.equal(result.models,6);assert.equal(result.runs,23);assert.equal(result.restores,161);assert.equal(result.advancing,138);assert.equal(result.terminal,23);
assert.equal(plan.runs.length,23);assert.equal(new Set(plan.runs.map(r=>r.modelIdentity)).size,6);
...
const build=read(p+'REL_DIMENSIONS_BUILD_REV1.json');assert.equal(build.status,'PASS');for(const a of build.artifacts)assert.equal(sha(a.path),a.sha256,a.path);
...
const workers=Array.from({length:4},(_,i)=>p+'relDimensions-public-workers-rev1/'+i+'.json');
...
const preservationPath=p+'REL_DIMENSIONS_PRESERVATION_REV1.json';for(const a of read(preservationPath).artifacts)assert.equal(sha(a.path),a.sha256,a.path);
const files=[preservationPath,p+'REL_DIMENSIONS_IMPLEMENTATION_FINDINGS.md',...workers,planPath,resultPath,p+'REL_DIMENSIONS_TESTS_REV2.json',p+'REL_DIMENSIONS_REFERENCE_TESTS_REV1.json',p+'REL_DIMENSIONS_BUILD_REV1.json',p+'CAMPAIGN3_REL_DIMENSIONS_QUALIFICATION.md','scripts/check-rel-dimensions-closure.mjs','docs/formal/REL_DIMENSIONS_PUBLIC_CONTRACT.md'];
const closure={status:'PASS',contract:'rel-dimensions-public/0.1-candidate',newModels:6,models:6,runs:23,prefixes:161,advancing:138,terminal:23,tests:23,referenceTests:328,artifacts:files.map(path=>({path,sha256:sha(path)}))};
const path=p+'REL_DIMENSIONS_CLOSURE_REV1.json';
if(process.argv.includes('--write'))fs.writeFileSync(path,JSON.stringify(closure,null,2)+'\n',{flag:'wx'});
assert.deepEqual(read(path),closure,'RelDimensions closure evidence drift');
console.log('PASS relDimensions:6 models/23 runs/161 prefixes;23+328 tests; counters1220/0.');
