...
const output='docs/planning/CAMPAIGN3_IDENTITY_ABLATION_REVIEW_REV2.json';
...
const paths=[...files('src').filter(p=>p.endsWith('.ts')),'scripts/review-campaign3-identity-ablation-rev2.mjs','scripts/cognitive-model-declarations.mjs','docs/planning/campaign2-task-cognitive-model/FREEZE.json'];
...
try{
 ...
 for(const seed of [0,1,2,3]){
  ...
  for(const recipe of ['baseline','standing-access-off','standing-integer']){
   const input=data();input.runSeed[31]=seed;const source=t.source(recipe),run=await createCognitiveRun(await prepareCognitiveModel(source),input);const prefixes=[];while(await run.settleNextInstant()){const p=run.snapshot();prefixes.push({clock:String(p.clock),state:encode(decode(p.state)),history:decode(p.state).items.map(v=>f(v,2)).filter(v=>v.schema?.typeId===414n).map(encode)});}
  ...
  const a=runs.baseline,b=runs['standing-access-off'];assert.deepEqual(a.prefixes.filter(p=>Number(p.clock)<6),b.prefixes.filter(p=>Number(p.clock)<6));assert.deepEqual(a.choices.slice(0,3),b.choices.slice(0,3));const prior=a.prefixes.find(p=>p.clock==='5');assert(prior.history.length>0);const am=new Map(a.actualDraws.map(h=>[encode(f(decode(Buffer.from(h,'hex')),1)),h])),bm=new Map(b.actualDraws.map(h=>[encode(f(decode(Buffer.from(h,'hex')),1)),h]));let sharedAddresses=0;for(const [k,v]of am)if(bm.has(k)){assert.equal(v,bm.get(k));sharedAddresses++;}pairs.push({seed,matchedPreProbeStateAndHistory:true,firstThreeDecisionsEqual:true,sharedAddressesWithIdenticalFullDraws:sharedAddresses,probeIntact:f(decode(Buffer.from(a.choices[3],'hex')),8).value.toString(),probeAblated:f(decode(Buffer.from(b.choices[3],'hex')),8).value.toString(),probeProbabilityDifference:encode(f(decode(Buffer.from(a.choices[3],'hex')),2))!==encode(f(decode(Buffer.from(b.choices[3],'hex')),2)),historyEqual:JSON.stringify(a.history)===JSON.stringify(b.history),stateEqual:a.state===b.state,decisionDataEqual:JSON.stringify(a.choices)===JSON.stringify(b.choices),actualDrawsEqual:JSON.stringify(a.actualDraws)===JSON.stringify(b.actualDraws)});console.log(JSON.stringify(pairs.at(-1)));
 }
