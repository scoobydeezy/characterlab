...
const root=new URL('../',import.meta.url),folder=new URL('docs/planning/campaign2-probe-successor-model/',root);
const hash=b=>crypto.createHash('sha256').update(b).digest('hex');
const prior=['campaign2-first-model','campaign2-trace-model','campaign2-probe-model'].flatMap(dir=>fs.readdirSync(new URL(`docs/planning/${dir}/`,root)).sort().map(name=>{const path=`docs/planning/${dir}/${name}`;return {path,sha256:hash(fs.readFileSync(new URL(path,root)))};}));
...
try{
 ...
 const {probeSuccessorReview}=await server.ssrLoadModule('/src/campaign2/probeSuccessorReview.ts');
 const compiled=await probeSuccessorReview(),source=compiled.source,model=compiled.modelIdentity;
 const oldFolder=new URL('docs/planning/campaign2-probe-model/',root),oldBytes=n=>Buffer.from(fs.readFileSync(new URL(n+'.cenc.hex',oldFolder),'utf8').trim(),'hex');
 ...
 assert.deepEqual(changedFields,['1']);
 ...
 assert(delta.every(d=>d.byteIdentical&&d.added.length===0));
 ...
 for(const [name,bytes] of Object.entries(artifacts)){
  ...
  const unchanged=Buffer.from(bytes).equals(oldBytes(name));assert.equal(unchanged,name!=='model-identity');
 ...
 console.log(JSON.stringify({modelDigest:report.modelDigest,changedModelFields:changedFields,addedSlot0:0,addedSlot5:0,preservedFiles:prior.length,status:report.status}));
