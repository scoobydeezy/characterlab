...
const output='docs/planning/COGNITIVE_SOURCE_SUBSTITUTIONS_REV2.json';assert(!fs.existsSync(output));
...
const paths=[...new Set([...mutants.map(m=>m[1]),...Object.values(tests)]),'scripts/prove-cognitive-substitutions-rev2.mjs'],fp=()=>paths.map(path=>({path,sha256:createHash('sha256').update(fs.readFileSync(path)).digest('hex')})),before=fp();
