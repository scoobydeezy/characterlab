...
const source='docs/planning/GA_STATE_ROOT_KEY_INVENTORY_REV2.json',output='docs/planning/GA_STATE_ROOT_KEY_REVIEW_REV2.json';assert(!fs.existsSync(output));const original=JSON.parse(fs.readFileSync(source));
function validate(p){
 ...
 for(const f of ['Nodes','Rows'])assert.deepEqual([field('AssociationGraph',f).min,field('AssociationGraph',f).max],[0,30]);
 assert.deepEqual([field('AssociationMassRow','Masses').min,field('AssociationMassRow','Masses').max],[0,30]);
