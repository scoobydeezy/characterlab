...
const workerRoot='docs/planning/personstate-public-workers-rev2';
const p='docs/planning/',planPath=p+'PERSONSTATE_PUBLIC_PLAN_REV2.json',resultPath=p+'PERSONSTATE_PUBLIC_RESULT_REV2.json';
...
try {
 ...
 if(process.argv.includes('--plan')) {
  ...
  const experiment=await ids.createExperimentIdentity('corpus/0.29.0',m.VERSION,'personstate-public-harness/0.2-candidate');
  ...
  const paths=['docs/formal/PERSONSTATE_PUBLIC_CONTRACT.md','docs/formal/PERSONSTATE_PUBLIC_ALLOCATION_TABLE.json',p+'campaign3-personstate-model-rev2/FREEZE.json',...['Codecs','Model','Math','Runtime','Factory'].map(n=>'src/campaign3/personstate'+n+'.ts'),'src/test/personstateFixtures.ts','src/test/personstatePublic.test.ts',...['cognitiveTransforms','cognitiveMath','cognitiveChoice','cognitiveArbitration'].map(n=>'src/campaign2/'+n+'.ts'),'src/campaign3/affectFactorComparison.ts','scripts/qualify-personstate-public.mjs'];
