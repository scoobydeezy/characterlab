...
const planPath=p+'ATTRIBUTED_PUBLIC_PLAN_REV1.json',resultPath=p+'ATTRIBUTED_PUBLIC_RESULT_REV1.json';
const plan=read(planPath),result=read(resultPath),tests=read(p+'ATTRIBUTED_TESTS_REV2.json'),reference=read(p+'ATTRIBUTED_REFERENCE_TESTS_REV1.json');
assert.equal(result.status,'PASS');assert.equal(result.planSha256,sha(planPath));
assert.equal(result.models,6);assert.equal(result.runs,30);assert.equal(result.restores,120);assert.equal(result.advancing,90);assert.equal(result.terminal,30);
assert.equal(plan.runs.length,30);assert.equal(new Set(plan.runs.map(r=>r.modelIdentity)).size,6);
...
assert.equal(tests.numPassedTests,34);assert.equal(tests.numFailedTests,0);
assert.equal(reference.numPassedTests,328);assert.equal(reference.numFailedTests,0);
assert.equal(read(p+'ATTRIBUTED_BUILD_REV2.json').status,'PASS');
const freeze=read(p+'campaign3-attributed-model-rev1/FREEZE.json');
...
const allocation=read('docs/formal/ATTRIBUTED_PUBLIC_ALLOCATION_TABLE.json');
assert.equal(allocation.namespace,1172);assert.deepEqual(allocation.records.map(r=>r.typeId),Array.from({length:17},(_,i)=>1131+i));
assert.equal(freeze.contractSha256,sha('docs/formal/ATTRIBUTED_PUBLIC_CONTRACT.md'));assert.equal(freeze.allocationSha256,sha('docs/formal/ATTRIBUTED_PUBLIC_ALLOCATION_TABLE.json'));
const workers=Array.from({length:4},(_,i)=>p+'attributed-public-workers-rev1/'+i+'.json');
...
const preservationPath=p+'ATTRIBUTED_DEVELOPMENT_PRESERVATION_REV1.json';
for(const a of read(preservationPath).artifacts)assert.equal(sha(a.path),a.sha256,a.path);
const files=[...workers,planPath,resultPath,preservationPath,p+'ATTRIBUTED_TESTS_REV2.json',p+'ATTRIBUTED_REFERENCE_TESTS_REV1.json',p+'ATTRIBUTED_BUILD_REV2.json',p+'CAMPAIGN3_ATTRIBUTED_QUALIFICATION.md',p+'ATTRIBUTED_DEVELOPMENT_FINDINGS.md',p+'ATTRIBUTED_TESTS_REV1.json',p+'ATTRIBUTED_BUILD_REV1.json','scripts/check-attributed-closure.mjs','docs/formal/ATTRIBUTED_PUBLIC_CONTRACT.md'];
const closure={status:'PASS',contract:'attributed-public/0.1-candidate',newModels:6,models:6,runs:30,prefixes:120,advancing:90,terminal:30,tests:34,referenceTests:328,artifacts:files.map(path=>({path,sha256:sha(path)}))};
const path=p+'ATTRIBUTED_CLOSURE_REV1.json';
if(process.argv.includes('--write'))fs.writeFileSync(path,JSON.stringify(closure,null,2)+'\n',{flag:'wx'});
assert.deepEqual(read(path),closure,'Attributed closure evidence drift');
console.log('PASS attributed:6 models/30 runs/120 prefixes;34+328 tests; counters1147/0.');
