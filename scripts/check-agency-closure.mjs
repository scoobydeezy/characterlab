...
const planPath=p+'AGENCY_PUBLIC_PLAN_REV3.json',resultPath=p+'AGENCY_PUBLIC_RESULT_REV3.json';
const plan=read(planPath),result=read(resultPath),tests=read(p+'AGENCY_TESTS_REV3.json'),reference=read(p+'AGENCY_REFERENCE_TESTS_REV1.json');
...
assert.equal(tests.numPassedTests,38);assert.equal(tests.numFailedTests,0);
assert.equal(reference.numPassedTests,328);assert.equal(reference.numFailedTests,0);
assert.equal(read(p+'AGENCY_BUILD_REV3.json').status,'PASS');
...
const phase=read(p+'AGENCY_PHASE_FAILURE_REV1.json');
assert.equal(phase.actualIntentPhase,80);assert.equal(phase.requiredIntentPhase,70);
assert.equal(phase.modelIdentity,read(p+'campaign3-agency-model-rev2/FREEZE.json').models[0].modelIdentity);
const second=read(p+'agency-rev2/PRESERVATION.json');
for(const a of second.artifacts)assert.equal(sha(a.path),a.sha256,a.path);
for(const a of read(p+'AGENCY_PUBLIC_PLAN_REV2.json').artifacts){const copy=second.artifacts.find(x=>x.original===a.path);assert(copy,a.path);assert.equal(copy.sha256,a.sha256,a.path);}
...
const files=[planPath,resultPath,p+'AGENCY_TESTS_REV3.json',p+'AGENCY_REFERENCE_TESTS_REV1.json',p+'AGENCY_BUILD_REV3.json',p+'AGENCY_UNRECEIVED_REPORT_FAILURE_REV1.json',p+'agency-rev1/PRESERVATION.json',p+'agency-rev2/PRESERVATION.json',p+'AGENCY_PHASE_FAILURE_REV1.json',p+'CAMPAIGN3_AGENCY_QUALIFICATION.md',p+'AGENCY_IMPLEMENTATION_FINDINGS.md','scripts/check-agency-closure.mjs'];
const closure={status:'PASS',contract:'agency-public/0.3-candidate',models:5,runs:19,prefixes:71,advancing:52,terminal:19,tests:38,referenceTests:328,preservedFailures:['unreceived report shifted a later nonrecipient occurrence ID in revision1','intent emitted at80 despite required70 in revisions1/2'],artifacts:files.map(path=>({path,sha256:sha(path)}))};
const path=p+'AGENCY_CLOSURE_REV2.json';
...
console.log('PASS agency:5 models/19 public runs/71 prefixes;38+328 tests; failed first cohort preserved.');
