...
const output='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV12.json';assert(!fs.existsSync(output));
const inputs=['GA_SURVIVING_CHILD_CARRIER_REV3','GA_GOAL_RESULT_CARRIER_REV3','GA_FORMATION_PROTOCOL_CARRIER_REV2','GA_RECOLLECTION_CARRIER_REV3','GA_BODY_PIPELINE_CARRIER_REV3','GA_VISUAL_PIPELINE_CARRIER_REV2','GA_STATE_ROOT_KEY_INVENTORY_REV2','GA_PRIOR_FEEDBACK_CARRIER_REV1','GA_LOCAL_RESERVE_PUBLIC_CARRIER_REV1','GA_REQUESTED_SOURCE_CARRIER_REV2','GA_SOURCE_PROFILE_DECLARATIONS_REV1','GA_SOURCE_STAGE_CLOSURE_REV1','GA_ACQUISITION_RECALL_STAGE_CLOSURE_REV1','GA_PRIOR_CONCERN_STAGE_CLOSURE_REV1','GA_GOAL_ATTRIBUTION_STAGE_CLOSURE_REV2'].map(n=>'docs/planning/'+n+'.json');
const packets=inputs.map(p=>JSON.parse(fs.readFileSync(p))),records=packets.flatMap(p=>p.records),names=new Set(records.map(r=>r.name));assert.equal(names.size,145);
const identities={SceneObjectReferent:{namespace:1002},EventRoleId:{namespace:1003},LocalReserveId:{symbolicFamily:'LocalReserveId'},ConcernOccurrenceId:{namespace:1130},ObserverId:{namespace:1000},CharacterId:{namespace:1002,validator:'validator/character-qualification'},SemanticReferentId:{namespace:1002,validator:'validator/maintenance-goal-qualification'},DefinitionId:{namespace:1027},ObservationId:{namespace:1115},ExperienceId:{namespace:1106},SelectionOccurrenceId:{namespace:1143},TransitionKindId:{namespace:1009},MutationAuthorityId:{namespace:1025},ObservationChannelId:{namespace:1005},AcquisitionOccurrenceId:{symbolicFamily:'AcquisitionOccurrenceId'},InteroceptiveSignalId:{symbolicFamily:'InteroceptiveSignalId'},GoalOutcomeAssessmentId:{symbolicFamily:'GoalOutcomeAssessmentId'},RetainedAttributionResultId:{symbolicFamily:'RetainedAttributionResultId'},ProposedRecollectionOccurrenceId:{symbolicFamily:'ProposedRecollectionOccurrenceId'}};
const inherited={TaskWorkspace:'existing381',TaskAppraisal:'existing384',WorldEventTruth:'existing210',ReserveAnchor:'existing454',ReserveReplenishmentResult:'existing479',TaskConcernResponse:'existing386',PerceptualReferentId:'existing212',PerceptualEventReferentId:'existing213',PreRecognitionSemanticExperience:'existing227',EmbodiedLevelObservation:'existing461',EmbodiedSample:'existing461/463',CanonicalRecordSchemaRef:'existing254',StatePathPattern:'existing149',CurrentDetectionId:'existing214',CurrentEventDetectionId:'existing215',PerceptualTrackTransition:'existing217',PerceptualEventTransition:'existing219',EventRoleEvidence:'existing223',PerceivedBindingEvidence:'existing224',CausalRoleEvidence:'existing240',AttentionUnitKey:'existing530',SelectedEvidenceView:'existing534'};
...
fs.writeFileSync(output,JSON.stringify({version:'ga-identity-role-closure/0.12-draft',status:'SYMBOLIC ROLE CROSSWALK; PUBLIC COMPILER AND ALLOCATION OPEN',recordCount:records.length,positions,boundaries,stateMapKeyRoles,stateMapKeyDisposition:'Symbolic six scalar new-root key roles plus composite physical key recursion; actual compiler and inherited root coverage remain separate.',sources:[...inputs,'docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV12.md','scripts/build-ga-identity-role-closure-rev12.mjs'].map(fp)},null,2)+'\n');
