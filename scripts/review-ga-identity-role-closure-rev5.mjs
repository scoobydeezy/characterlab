...
const source='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV5.json',output='docs/planning/GA_IDENTITY_ROLE_REVIEW_REV5.json';assert(!fs.existsSync(output));const original=JSON.parse(fs.readFileSync(source));
function validate(p){
 assert.equal(p.recordCount,116);assert.equal(p.positions.length,112);assert.equal(p.boundaries.length,162);assert.equal(new Set(p.positions.map(x=>x.path)).size,112);
