...
const output='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV2.json';assert(!fs.existsSync(output));
const inputs=['GA_SURVIVING_CHILD_CARRIER_REV3','GA_GOAL_RESULT_CARRIER_REV2','GA_FORMATION_PROTOCOL_CARRIER_REV2','GA_RECOLLECTION_CARRIER_REV3','GA_BODY_PIPELINE_CARRIER_REV3','GA_VISUAL_PIPELINE_CARRIER_REV1'].map(n=>'docs/planning/'+n+'.json');
const packets=inputs.map(p=>JSON.parse(fs.readFileSync(p))),records=packets.flatMap(p=>p.records),names=new Set(records.map(r=>r.name));assert.equal(names.size,82);
...
const primitives=new Set(['Instant','Glyph','VersionText','Boolean','False','Rational','NonnegativeRational','OriginalAddress','ViewOrdinal','FieldOrdinal','SourceBound','ViewCount','Cell','UnitRational','PositiveUnitRational']);
...
fs.writeFileSync(output,JSON.stringify({version:'ga-identity-role-closure/0.2-draft',status:'SYMBOLIC ROLE CROSSWALK; PUBLIC COMPILER AND ALLOCATION OPEN',recordCount:records.length,positions,boundaries,stateMapKeyRoles:[],stateMapKeyDisposition:'Pending exact root key inventory; composite record keys recurse, never scalar roles.',sources:[...inputs,'docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV2.md','scripts/build-ga-identity-role-closure-rev2.mjs'].map(fp)},null,2)+'\n');
