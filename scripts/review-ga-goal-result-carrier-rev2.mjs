...
const source='docs/planning/GA_GOAL_RESULT_CARRIER_REV2.json',output='docs/planning/GA_GOAL_RESULT_CARRIER_REVIEW_REV2.json';assert(!fs.existsSync(output));
...
function validate(p){
 ...
 for(const n of ['GoalClosedInterval','GoalDistanceAssessment','GoalOutcomeAssessment','MaintenanceGoalSpec','AdoptedMaintenanceGoal','DefinitionId','RetainedViewAddress']){assert(!carry.has(n),'carry leaks '+n);assert(!join.has(n),'join leaks '+n);}
...
const cases=[
 ...
 ['indirect-numeric-qualification',p=>record(p,'GoalQualifies').fields.push({name:'Distance',type:{kind:'ref',name:'GoalClosedInterval'}})],
 ...
 ['mutable-goal-baseline',p=>record(p,'AdoptedMaintenanceGoal').fields.push({name:'Desired',type:{kind:'ref',name:'GoalClosedInterval'}})],
