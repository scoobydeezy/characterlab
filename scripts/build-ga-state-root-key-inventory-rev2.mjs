...
const output='docs/planning/GA_STATE_ROOT_KEY_INVENTORY_REV2.json';assert(!fs.existsSync(output));
...
record('AssociationMassRow',{Masses:list('LatticeMass',0,30)});
record('AssociationGraph',{Nodes:list('PerceptualReferentId',0,30),Rows:list('AssociationMassRow',0,30),LastUpdatedAt:'Instant'});
...
fs.writeFileSync(output,JSON.stringify({version:'ga-state-root-key-inventory/0.2-draft',status:'SYMBOLIC ROOT/KEY INVENTORY; PUBLIC COMPILER OPEN',records,roots,stateMapKeyRoles,external,invariants:{cohort:'Exactly one compiled character/observer; key and every nested holder agree.',graph:'Canonical unique keys, square rows, zero diagonal, nonnegative bounded lattice and row sums.',perception:'No new1031 perception member; no copied SEM counter or full observation history.',protocol:'No map selector or cognitive role/read/family on singleton.',empty:'Initialized owner values remain present after complete loss; old roots unchanged.'},sources:['docs/planning/GA_STATE_ROOT_KEY_INVENTORY_REV2.md','docs/planning/GA_SURVIVING_CHILD_CARRIER_REV3.json','docs/planning/GA_GOAL_RESULT_CARRIER_REV2.json','docs/planning/GA_RECOLLECTION_CARRIER_REV3.json','docs/planning/GA_FORMATION_PROTOCOL_CARRIER_REV2.json'].map(fp)},null,2)+'\n');
