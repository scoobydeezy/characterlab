...
const root='docs/planning/campaign3-embodied-model-rev2',folder='docs/planning/embodied-execution-rev3';assert(!fs.existsSync(folder));
...
try{const api=await server.ssrLoadModule('/src/campaign3/embodiedFactory.ts'),{decodeEmbodied:decode}=await server.ssrLoadModule('/src/campaign3/embodiedCodecs.ts');
 ...
 fs.writeFileSync(folder+'/REVIEW.json',JSON.stringify({status:'SEVEN PUBLIC EXECUTIONS MATCH FROZEN EXPECTATIONS; WHOLE VECTOR REVIEW OPEN',results,artifacts,sources,cohort:fp(root+'/FREEZE.json'),expectations:fp('docs/planning/CAMPAIGN3_EMBODIED_PROFILE_EXPECTATIONS_REV1.json'),script:fp('scripts/review-embodied-execution-rev3.mjs'),preservedReceiptEntries:old.checks.filter(p=>!changed.includes(p.path)).length,reviewedSourceChanges:changed.map(fp),limits:['Denied/unavailable hidden reserve expectation is deliberately not read or reconstructed by the runtime.','Frozen historical expectation bytes remain unchanged.','Bounded public execution does not establish EMB-M..O, BODY/MULTISOURCE, action knowledge or final Need ownership.']},null,2)+'\n');console.log(JSON.stringify(results));
