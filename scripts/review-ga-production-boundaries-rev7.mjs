...
const output='docs/planning/GA_PRODUCTION_BOUNDARIES_REVIEW_REV7.json';assert(!fs.existsSync(output));
...
const faults=[['owner-protocol-ignores-actual-loss','src/campaign3/significantFormationSettlement.ts','src/test/concernSignificanceSourceComposition.test.ts','const survivors=memory.map(a=>a.id)','const survivors=combined.map(a=>a.id)','SC-AC'],["policy-cues-gated-by-selection","src/campaign3/observationExecutionPolicy.ts","src/test/observationExecutionPolicy.test.ts","bodyRecall:use.bodyCue","bodyRecall:use.bodyCue&&use.bodySelection","OEP-B"],["policy-reuses-baseline-slot","src/campaign3/observationExecutionPolicy.ts","src/test/observationExecutionPolicy.test.ts","resolveKind(copyId(id))","resolveKind(copyId(slot==='goalBaselineRecall'?copies.bodyRecall!:id))","OEP-C"],["policy-ignores-kind","src/campaign3/observationExecutionPolicy.ts","src/test/observationExecutionPolicy.test.ts","resolveKind(copyId(id))!==kinds[slot]","false","OEP-E"],["policy-resolver-mutates-bound-id","src/campaign3/observationExecutionPolicy.ts","src/test/observationExecutionPolicy.test.ts","resolveKind(copyId(id))","resolveKind(id)","OEP-F"],["age-control-uses-significance","src/campaign3/significantFormationSettlement.ts","src/test/ordinaryMemoryBatch.test.ts","prepare(input,ageOnly)","prepare(input,retainSignificanceFirst)","OMB-G"],["use-control-uses-significance","src/campaign3/significantFormationSettlement.ts","src/test/ordinaryMemoryBatch.test.ts","prepare(input,useOnly)","prepare(input,retainSignificanceFirst)","OMB-G"],["shared-control-uses-significance","src/campaign3/significantFormationSettlement.ts","src/test/ordinaryMemoryBatch.test.ts","prepare(input,retainSharedProtectionTier)","prepare(input,retainSignificanceFirst)","OMB-G"],["control-erases-ignored-significance","src/campaign3/significantFormationSettlement.ts","src/test/ordinaryMemoryBatch.test.ts","outcomeSignificanceDirections:[...original.outcomeSignificanceDirections]","outcomeSignificanceDirections:[]","OMB-H"],
  [
