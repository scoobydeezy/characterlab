...
const output='docs/planning/EMBODIED_RUNTIME_MUTANTS_REV1.json';assert(!fs.existsSync(output));
...
fs.writeFileSync(output,JSON.stringify({status:'NINE CURRENT-SOURCE ALTERNATIVES DETECTED',results,sources:['src/campaign3/embodiedRuntime.ts','src/campaign3/embodiedMath.ts','src/campaign3/embodiedTrace.ts','src/campaign3/embodiedAdmission.ts','scripts/review-embodied-runtime-mutants.mjs'].map(fp),cohort:fp(folder+'/FREEZE.json'),limits:['Fault sensitivity, not a reduction verdict.','No receiving/action-knowledge or whole BODY claim.']},null,2)+'\n');console.log(JSON.stringify(results.map(r=>({name:r.name,detectedBy:r.detectedBy,failure:r.failure}))));
