...
const source='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV1.json',output='docs/planning/GA_IDENTITY_ROLE_REVIEW_REV1.json';assert(!fs.existsSync(output));const original=JSON.parse(fs.readFileSync(source));
function validate(p){
 assert.equal(p.recordCount,66);assert.equal(p.positions.length,72);assert.equal(p.boundaries.length,82);assert.equal(new Set(p.positions.map(x=>x.path)).size,72);
 ...
 assert.deepEqual(p.stateMapKeyRoles,[]);assert(p.stateMapKeyDisposition.startsWith('Pending'));assert(p.status.includes('OPEN'));
...
const fp=path=>({path,sha256:createHash('sha256').update(fs.readFileSync(path)).digest('hex')});fs.writeFileSync(output,JSON.stringify({status:'SYMBOLIC IDENTITY ROLE REVIEW PASS; PUBLIC GATES OPEN',records:66,identityPositions:72,recordBoundaries:82,faults,sources:[source,'scripts/review-ga-identity-role-closure.mjs'].map(fp),limits:['Structural crosswalk only; actual canonical role tables and domain/holder checks await final public closure.','RetainedEncodingUnit and state-root keys are explicitly pending, not inferred from this report.']},null,2)+'\n');
