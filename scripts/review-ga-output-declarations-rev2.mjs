...
const path='docs/planning/GA_OUTPUT_DECLARATIONS_REV3.json',out='docs/planning/GA_OUTPUT_DECLARATIONS_REVIEW_REV2.json';
assert(!fs.existsSync(out));
const packet=JSON.parse(fs.readFileSync(path)),topology=JSON.parse(fs.readFileSync('docs/planning/GA_COMPOSED_LOGICAL_TOPOLOGY_REV2.json'));
...
function validate(p){
 const physical=row(p,'local-reserve-replenishment','LocalReserveReplenishmentResult');assert.equal(physical.min,1);assert.equal(physical.max,1);assert.equal(physical.identity,'BorrowedOnly');assert.deepEqual(physical.fresh,[]);
...
validate(packet);
const faults=[['replenishment-invents-occurrence',p=>{const r=row(p,'local-reserve-replenishment','LocalReserveReplenishmentResult');r.identity='FreshOwned';r.fresh=['ObservationId'];}],
