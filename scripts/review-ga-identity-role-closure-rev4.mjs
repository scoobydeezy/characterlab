...
const source='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV4.json',output='docs/planning/GA_IDENTITY_ROLE_REVIEW_REV4.json';assert(!fs.existsSync(output));const original=JSON.parse(fs.readFileSync(source));
function validate(p){
 assert.equal(p.recordCount,111);assert.equal(p.positions.length,106);assert.equal(p.boundaries.length,152);assert.equal(new Set(p.positions.map(x=>x.path)).size,106);
 ...
 assert(p.boundaries.some(x=>x.path==='PositiveEventChildEvidence.Encoding'&&x.record==='RetainedEncodingUnit'&&x.mode==='RecurseDeclaredRecord')); for(const [name,id] of [['CurrentDetectionId',214],['CurrentEventDetectionId',215],['PerceptualTrackTransition',217],['PerceptualEventTransition',219],['EventRoleEvidence',223],['PerceivedBindingEvidence',224],['CausalRoleEvidence',240],['AttentionUnitKey',530],['SelectedEvidenceView',534]]){const b=p.boundaries.filter(x=>x.record===name);assert(b.length>0);for(const x of b)assert.equal(x.source,'existing'+id);} assert.deepEqual(p.stateMapKeyRoles,JSON.parse(fs.readFileSync('docs/planning/GA_STATE_ROOT_KEY_INVENTORY_REV1.json')).stateMapKeyRoles);assert(p.stateMapKeyDisposition.startsWith('Symbolic'));assert(p.status.includes('OPEN'));
