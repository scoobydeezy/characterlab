...
const source='docs/planning/GA_COMPOSED_LOGICAL_TOPOLOGY_REV1.json',output='docs/planning/GA_LOGICAL_TOPOLOGY_REVIEW_REV1.json';assert(!fs.existsSync(output));const original=JSON.parse(fs.readFileSync(source));
function validate(p){
 const byName=new Map(p.stages.map(s=>[s.name,s]));assert.equal(byName.size,66);assert.equal(p.stages.length,66);assert.equal(p.wholeModelWorkCeiling,null);assert.deepEqual(p.numericAllocations,[]);
...
validate(original);
const row=(p,n)=>p.stages.find(s=>s.name===n),faults=[
