...
const mutants=[
 ...
 {name:'reverse-target-gate-read-evidence',from:'reads.push(...projection.actualReadRecords());',to:'reads.push(...projection.actualReadRecords().reverse());'},
 {name:'omit-gate-read-evidence',from:'reads.push(...projection.actualReadRecords());',to:'reads.push(...projection.actualReadRecords().slice(0,1));'},
];
...
fs.writeFileSync(new URL('docs/planning/CAMPAIGN2_BATCH_MUTATION_PROOF.json',root),JSON.stringify({status:'COMPONENT PASS',sourceFingerprints:[file,test].map(path=>({path,sha256:hash(path)})),baseline,mutants:results,
