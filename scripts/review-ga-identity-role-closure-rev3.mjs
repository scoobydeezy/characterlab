...
const source='docs/planning/GA_IDENTITY_ROLE_CLOSURE_REV3.json',output='docs/planning/GA_IDENTITY_ROLE_REVIEW_REV3.json';assert(!fs.existsSync(output));const original=JSON.parse(fs.readFileSync(source));
function validate(p){
 assert.equal(p.recordCount,102);assert.equal(p.positions.length,98);assert.equal(p.boundaries.length,141);assert.equal(new Set(p.positions.map(x=>x.path)).size,98);
