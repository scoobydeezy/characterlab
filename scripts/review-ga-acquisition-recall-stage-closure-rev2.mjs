...
const input='docs/planning/GA_ACQUISITION_RECALL_STAGE_CLOSURE_REV1.json',output='docs/planning/GA_ACQUISITION_RECALL_STAGE_REVIEW_REV2.json';assert(!fs.existsSync(output));const original=JSON.parse(fs.readFileSync(input));
function validate(p){
 ...
 for(const s of p.stages){assert.equal(s.projection,'Required PRJ/IDN via field1 ObserverId after actual producer admission');for(const target of s.next){assert(byName.has(target));assert(byName.get(target).phase>=s.phase);}assert(!s.reads.includes('FormationGovernanceState'));if(s.phase!==140&&!s.name.endsWith('-rank'))assert.deepEqual(s.reads,[]);if(s.phase!==140)assert.deepEqual(s.writes,[]);else{assert.deepEqual(s.outputs,[]);assert.deepEqual(s.next,[]);assert.equal(s.allocation,'None');assert(s.batch.includes('no sibling reads'));}assert.deepEqual(s.route,s.phase===140||s.name.endsWith('-acquisition-evidence')?['route/character-learning']:[]);}
...
const faults=[
 ['encoder-physical-read',p=>stage(p,'current-visual-encoding').reads=['LocalReserveState']],
