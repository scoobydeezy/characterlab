...
const output='docs/planning/embodied-receiving-execution-rev4';assert(!fs.existsSync(output));
...
try{
 ...
 for(const specimen of cases){const modelName=specimen.model??'baseline',read=n=>new Uint8Array(Buffer.from(fs.readFileSync(cohort+'/'+modelName+'/'+n+'.cenc.hex','utf8').trim(),'hex'));
  ...
  const files=[];for(const [name,bytes] of Object.entries(artifacts)){check(specimen.name+' current bytes preserved '+name,()=>assert.equal(Buffer.from(bytes).toString('hex'),fs.readFileSync('docs/planning/embodied-receiving-execution-rev3/'+specimen.name+'/'+name+'.cenc.hex','utf8').trim()));const path=dir+'/'+name+'.cenc.hex';fs.writeFileSync(path,Buffer.from(bytes).toString('hex')+'\n');files.push(fp(path));}
