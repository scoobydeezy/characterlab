...
const output='docs/planning/ENCODING_ACCESS_MATH_REVIEW_REV1.json';assert(!fs.existsSync(output));
const changes=[
...
const fp=p=>({path:p,sha256:createHash('sha256').update(fs.readFileSync(p)).digest('hex')});const tests=JSON.parse(fs.readFileSync('docs/planning/ENCODING_ACCESS_MATH_TESTS_REV1.json'));assert(tests.success&&tests.numPassedTests===12);
fs.writeFileSync(output,JSON.stringify({status:'EAM-A..L QUALIFIED AT MATHEMATICS COMPONENT SCOPE',tests:12,faults:results,baseline,sources:['src/campaign3/encodingAccessMath.ts','src/test/encodingAccessMath.test.ts','docs/formal/ENCODING_ACCESS_MATH.md','docs/planning/GENERAL_ATTENTION_CLOSURE_PLAN.md','scripts/review-encoding-access-math.mjs'].map(fp),evidence:fp('docs/planning/ENCODING_ACCESS_MATH_TESTS_REV1.json'),limits:['No public evidence producer, canonical record, persistent writer, source/subject binding, retrieval event or corpus promotion. General attention remains OPEN.']},null,2)+'\n');console.log({componentTests:12,detectedFaults:results.length,public:'NOT QUALIFIED'});
