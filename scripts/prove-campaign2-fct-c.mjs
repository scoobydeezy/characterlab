...
const mutations=['evaluation','learning'].flatMap(stage=>['state','event','trace','parent','content','registry','identityOrdinal','traceAncestry','perceivedSuccess'].map(field=>({stage,field,attempt:field==='state'?
...
async function run(test,mutant){
 ...
 const ctx=await startVitest('test',[test],{config:false,include:['src/test/**/*.test.ts'],run:true,watch:false,maxWorkers:1,minWorkers:1,reporters:[{onFinished(){}}],env:{CHARACTERLAB_FCT_C_PROJECTION_MUTANT:mutant?mutant.stage:'0'},...(mutant?{testNamePattern:'FCT-C1:'}:{})},
...
fs.writeFileSync(new URL('docs/planning/CAMPAIGN2_FCT_C_PROOF_REV4.json',root),JSON.stringify({status:'FCT-C refreshed split-scope evidence / AD-E11',sourceFingerprints:fingerprints,genericBaseline,projectionRejections,boundedBaseline,
 ...
 limitations:['Generic harness is not an admitted bounded factory profile.','Handler read evidence is observed directly; no generic canonical trace profile is invented.','Eighteen explicit expected-rejection substitutions, not merely arbitrary test failures.','Host admission/allocation/state validation remains outside the isolated semantic executor.','Occurrence payload remains a carried identity: named hidden-operand exclusion is not proof against arbitrary trusted host inspecting every nested byte.','No global factory/VAL/ADAPT/Campaign-2 verdict.']},null,2)+'\n');
console.log('FCT-C1 PASS; FCT-C2 PASS (18 rejected substitutions); FCT-C3 PASS.');
process.exitCode=0;
